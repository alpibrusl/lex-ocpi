...
- `parse_result_post(req, extract_response_id)` — eMSP-side **pure** parser of the callback POST. Returns `PostCompleted({ response_id, result }) | PostBadResponseId(why) | PostBadBody(why)`. The user dispatches: on `PostCompleted` call `complete(actor, id, result)` (`[concurrent]`); on either error variant `result_post_handler_result` lifts to `OcpiErr(2001)`. Splitting the parser pure and the `tell` user-side is the same shape as slice 1's `command_handler` — the `route.Handler` type stays effect-free, the user wires the `[concurrent]` tail.
