...
fn buggy_target() -> cc.TargetConfig {
  { base_url: "http://localhost:9103/ocpi",
    token:    "any-token",
    version:  "2.2.1" }
}
...
fn suite() -> List[cc.Case] {
  [
    cc.expect_fail(ch.case_versions_returns_ok()),
    cc.expect_fail(ch.case_versions_data_is_list()),
    cc.expect_fail(ch.case_locations_list_returns_ok()),
    cc.expect_fail(ch.case_location_known_has_country_code()),
    cc.expect_fail(ch.case_locations_emits_x_total_count()),
    cc.expect_fail(ch.case_post_command_returns_accepted()),
  ]
}
