...
fn case_versions_returns_ok() -> cc.Case {
  { name: "GET /ocpi/versions returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_versions_data_is_list() -> cc.Case {
  { name: "GET /ocpi/versions data is a non-empty list", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    match client.get_with_token(cc.versions_url(cfg), cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
      Ok(data) => match data {
...
fn case_version_detail_returns_ok() -> cc.Case {
  { name: "GET /ocpi/{version} returns 1000 envelope (version detail)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_locations_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/{version}/locations returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_location_known_has_country_code() -> cc.Case {
  { name: "GET /ocpi/{version}/locations/LOC1 has country_code = NL", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
  ...
  match jv.get_field(data, "country_code") {
    ...
    Some(v) => match jv.as_str(v) {
      ...
      Some(s) => if s == want {
        ...
        CaseFail(str.concat("country_code = ", str.concat(s, str.concat(", want ", want))))
      },
    },
  }
}
...
fn case_location_known_has_evses() -> cc.Case {
  { name: "GET /ocpi/{version}/locations/LOC1 has non-empty `evses` array", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := cc.module_item_url(cfg, "locations", "LOC1")
    match client.get_with_token(url, cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
...
fn case_location_unknown_returns_2003() -> cc.Case {
  { name: "GET /ocpi/{version}/locations/LOC9 returns OCPI 2003", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    ...
    match client.get_with_token(url, cfg.token) {
      ...
      Err(OcpiError(r)) => if r.status_code == 2003 {
        ...
        CaseFail(str.concat("expected 2003, got ", int.to_str(r.status_code)))
      },
      Err(e) => CaseFail(str.concat("expected OCPI 2003, got ", cc.client_error_short(e))),
    }
  } }
}

fn case_unknown_path_returns_2000() -> cc.Case {
  { name: "GET /ocpi/wat returns OCPI 2000 (unknown route)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    ...
    match client.get_with_token(url, cfg.token) {
      ...
      Err(OcpiError(r)) => if r.status_code == 2000 {
        ...
        CaseFail(str.concat("expected 2000, got ", int.to_str(r.status_code)))
      },
      Err(e) => CaseFail(str.concat("expected OCPI 2000, got ", cc.client_error_short(e))),
    }
  } }
}

fn case_sessions_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/2.2.1/sessions returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_cdrs_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/2.2.1/cdrs returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_tariffs_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/2.2.1/tariffs returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_first_cdr_has_total_cost() -> cc.Case {
  { name: "GET /ocpi/2.2.1/cdrs first entry has total_cost.excl_vat", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    match client.get_with_token(cc.module_url(cfg, "cdrs"), cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
...
fn case_first_tariff_has_elements() -> cc.Case {
  { name: "GET /ocpi/2.2.1/tariffs first entry has non-empty `elements`", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    match client.get_with_token(cc.module_url(cfg, "tariffs"), cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
...
# ---- Auth negative cases --------------------------------------
fn assert_ocpi_status(res :: Result[jv.Json, client.ClientError], want :: Int, label :: Str) -> cc.CaseResult {
  match res {
    Ok(_) => CaseFail(str.concat(label, str.concat(": expected ", str.concat(int.to_str(want), ", got 1000-success")))),
    Err(OcpiError(r)) => if r.status_code == want {
      CasePass
    } else {
      CaseFail(str.concat(label, str.concat(": expected ", str.concat(int.to_str(want), str.concat(", got ", int.to_str(r.status_code))))))
    },
    Err(e) => CaseFail(str.concat(label, str.concat(": expected OCPI ", str.concat(int.to_str(want), str.concat(", got ", cc.client_error_short(e)))))),
  }
}

fn case_missing_auth_returns_2000() -> cc.Case {
  { name: "GET /ocpi/versions without Authorization returns OCPI 2000", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_malformed_auth_returns_2000() -> cc.Case {
  { name: "GET /ocpi/versions with Bearer auth returns OCPI 2000", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let req := http.with_header(client.base_request("GET", cc.versions_url(cfg)), "authorization", "Bearer wrong-scheme")
...
fn case_wrong_token_returns_2000() -> cc.Case {
  { name: "GET /ocpi/versions with wrong Token value returns OCPI 2000", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    assert_ocpi_status(client.get_with_token(cc.versions_url(cfg), "wrong-secret"), 2000, "wrong-token")
  } }
}
...
fn case_unsupported_version_returns_3002() -> cc.Case {
  { name: "GET /ocpi/9.9.9/locations returns OCPI 3002 (unsupported version)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cfg.base_url, "/9.9.9/locations")
    assert_ocpi_status(client.get_with_token(url, cfg.token), 3002, "unsupported-version")
  } }
}

fn case_malformed_json_returns_2001() -> cc.Case {
  { name: "POST /ocpi/2.2.1/commands/START_SESSION with malformed JSON returns 2001", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "commands"), "/START_SESSION")
    assert_ocpi_status(client.post_json(url, "{ not valid json", cfg.token), 2001, "malformed-json")
  } }
}

fn case_credentials_post_returns_ok() -> cc.Case {
  { name: "POST /ocpi/2.2.1/credentials returns 1000 envelope (handshake)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let body := jv.stringify(JObj([("token", JStr("emsp-secret")), ("url", JStr("http://localhost:9101/ocpi/versions")), ("roles", JList([JObj([("role", JStr("EMSP")), ("business_details", JObj([("name", JStr("Example eMSP"))])), ("country_code", JStr("DE")), ("party_id", JStr("ABC"))])]))]))
...
fn case_credentials_missing_field_returns_2001() -> cc.Case {
  { name: "POST /ocpi/2.2.1/credentials without `token` returns OCPI 2001", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let body := jv.stringify(JObj([("url", JStr("http://localhost:9101/ocpi/versions")), ("roles", JList([JObj([("role", JStr("EMSP")), ("business_details", JObj([("name", JStr("Example eMSP"))])), ("country_code", JStr("DE")), ("party_id", JStr("ABC"))])]))]))
    assert_ocpi_status(client.post_json(cc.module_url(cfg, "credentials"), body, cfg.token), 2001, "credentials-missing-token")
  } }
}
...
fn put_via_curl(url :: Str, token :: Str, body :: Str) -> [proc] Result[CurlResult, Str] {
  match proc.spawn("curl", ["-sS", "-X", "PUT", "-H", str.concat("Authorization: Token ", token), "-H", "content-type: application/json", "-w", "\n%{http_code}", "-d", body, url]) {
    ...
    Ok(r) => if r.exit_code == 0 {
      ...
      Err(str.concat("curl exit ", int.to_str(r.exit_code)))
    },
  }
}
...
fn parse_curl_output(out :: Str) -> Result[CurlResult, Str] {
  ...
  if n < 2 {
    Err("curl output: no status code line")
  } else {
...
fn case_put_token_returns_ok() -> cc.Case {
  { name: "PUT /ocpi/2.2.1/tokens/DE/ABC/RFID-A returns 1000 envelope (via curl)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "tokens"), "/DE/ABC/RFID-A")
    let body := jv.stringify(JObj([("uid", JStr("RFID-A")), ("type", JStr("RFID")), ("contract_id", JStr("DE-ABC-C12345-T")), ("issuer", JStr("Example Issuer")), ("valid", JBool(true)), ("whitelist", JStr("ALWAYS")), ("last_updated", JStr("2026-05-15T10:00:00Z"))]))
...
fn check_envelope_ok(body :: jv.Json) -> cc.CaseResult {
  match jv.get_field(body, "status_code") {
    ...
    Some(v) => match jv.as_int(v) {
      ...
      Some(n) => if n >= 1000 and n < 2000 {
        CasePass
      } else {
        CaseFail(str.concat("status_code=", int.to_str(n)))
      },
    },
  }
}
...
fn command_url(cfg :: cc.TargetConfig, command :: Str) -> Str {
  str.concat(cc.module_url(cfg, "commands"), str.concat("/", command))
}

fn start_session_body() -> Str {
  jv.stringify(JObj([("response_url", JStr("http://localhost:9101/ocpi/2.2.1/callback/cmd-1")), ("token", JObj([("country_code", JStr("DE")), ("party_id", JStr("ABC")), ("uid", JStr("RFID-A")), ("type", JStr("RFID")), ("contract_id", JStr("DE-ABC-C12345-T"))])), ("location_id", JStr("LOC1"))]))
}

fn case_post_command_returns_accepted() -> cc.Case {
  { name: "POST /ocpi/2.2.1/commands/START_SESSION returns ACCEPTED", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    match client.post_json(command_url(cfg, "START_SESSION"), start_session_body(), cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
...
fn callback_recorder_url() -> Str {
  "http://localhost:9101/callback"
}

fn case_command_callback_arrives() -> cc.Case {
  { name: "POST /commands/START_SESSION delivers CommandResult to response_url", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let body := jv.stringify(JObj([("response_url", JStr(callback_recorder_url())), ("token", JObj([("country_code", JStr("DE")), ("party_id", JStr("ABC")), ("uid", JStr("RFID-A")), ("type", JStr("RFID")), ("contract_id", JStr("DE-ABC-C12345-T"))])), ("location_id", JStr("LOC1"))]))
  ...
  match jv.get_field(data, "result") {
    ...
    Some(v) => match jv.as_str(v) {
      ...
      Some(s) => if s == want {
        ...
        CaseFail(str.concat("result=", str.concat(s, str.concat(", want ", want))))
      },
    },
  }
}
...
type RawResponse = { status :: Int, headers :: Map[Str, Str], body :: jv.Json }
...
fn case_locations_emits_x_total_count() -> cc.Case {
  { name: "GET /ocpi/2.2.1/locations carries X-Total-Count: 6", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_locations_limit_truncates_and_links_next() -> cc.Case {
  { name: "GET /ocpi/2.2.1/locations?limit=2 returns 2 items + Link rel=\"next\"", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "locations"), "?limit=2")
    match raw_get(url, cfg.token) {
      Err(m) => CaseFail(m),
      Ok(resp) => and_then(expect_list_len(resp.body, 2), fn () -> cc.CaseResult {
        expect_link_next(resp.headers, true)
      }),
    }
  } }
}
...
fn case_locations_last_page_has_no_link() -> cc.Case {
  { name: "GET /ocpi/2.2.1/locations?offset=4&limit=2 omits Link rel=\"next\"", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "locations"), "?offset=4&limit=2")
    match raw_get(url, cfg.token) {
      Err(m) => CaseFail(m),
      Ok(resp) => and_then(expect_list_len(resp.body, 2), fn () -> cc.CaseResult {
        expect_link_next(resp.headers, false)
      }),
    }
  } }
}
...
fn case_locations_date_from_filters() -> cc.Case {
  { name: "GET /ocpi/2.2.1/locations?date_from=2026-05-17 filters by last_updated", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "locations"), "?date_from=2026-05-17T00:00:00Z")
    match raw_get(url, cfg.token) {
      Err(m) => CaseFail(m),
      Ok(resp) => and_then(expect_header_eq(resp.headers, "x-total-count", "2"), fn () -> cc.CaseResult {
...
fn case_locations_date_to_filters() -> cc.Case {
  { name: "GET /ocpi/2.2.1/locations?date_to=2026-05-15T12:00:00Z filters by last_updated", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    let url := str.concat(cc.module_url(cfg, "locations"), "?date_to=2026-05-15T12:00:00Z")
    match raw_get(url, cfg.token) {
      Err(m) => CaseFail(m),
...
fn expect_header_eq(headers :: Map[Str, Str], name :: Str, want :: Str) -> cc.CaseResult {
  match map.get(headers, name) {
    None => CaseFail(str.concat("missing header: ", name)),
    Some(v) => if v == want {
      CasePass
    } else {
      CaseFail(str.concat(name, str.concat("=", str.concat(v, str.concat(", want ", want)))))
    },
  }
}
...
fn expect_list_len(body :: jv.Json, want :: Int) -> cc.CaseResult {
  match jv.get_field(body, "data") {
    ...
    Some(d) => match jv.as_list(d) {
      ...
      Some(l) => if list.len(l) == want {
        CasePass
      } else {
        CaseFail(str.concat("data has ", str.concat(int.to_str(list.len(l)), str.concat(" items, want ", int.to_str(want)))))
      },
    },
  }
}
...
fn case_chargingprofiles_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/{version}/chargingprofiles returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_hubclientinfo_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/{version}/hubclientinfo returns 1000 envelope", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_payments_list_returns_ok() -> cc.Case {
  { name: "GET /ocpi/{version}/payments returns 1000 envelope (v2.3.0)", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
...
fn case_v230_detail_lists_payments() -> cc.Case {
  { name: "GET /ocpi/2.3.0 version_detail advertises `payments`", run: fn (cfg :: cc.TargetConfig) -> [net, proc] cc.CaseResult {
    match client.get_with_token(cc.version_detail_url(cfg), cfg.token) {
      Err(e) => CaseFail(cc.client_error_short(e)),
...
fn check_detail_advertises(data :: jv.Json, want :: Str) -> cc.CaseResult {
  match jv.get_field(data, "endpoints") {
    ...
    Some(v) => match jv.as_list(v) {
      ...
      Some(l) => if list.fold(l, false, fn (acc :: Bool, ep :: jv.Json) -> Bool {
        if acc {
          true
        } else {
          match jv.get_field(ep, "identifier") {
            None => false,
        ...
        CaseFail(str.concat("`endpoints` does not advertise ", want))
      },
    },
  }
}
...
fn suite_v221() -> List[cc.Case] {
  [case_versions_returns_ok(), case_versions_data_is_list(), case_version_detail_returns_ok(), case_locations_list_returns_ok(), case_location_known_has_country_code(), case_location_known_has_evses(), case_location_unknown_returns_2003(), case_unknown_path_returns_2000(), case_sessions_list_returns_ok(), case_cdrs_list_returns_ok(), case_tariffs_list_returns_ok(), case_first_cdr_has_total_cost(), case_first_tariff_has_elements(), case_missing_auth_returns_2000(), case_malformed_auth_returns_2000(), case_wrong_token_returns_2000(), case_unsupported_version_returns_3002(), case_malformed_json_returns_2001(), case_credentials_post_returns_ok(), case_credentials_missing_field_returns_2001(), case_put_token_returns_ok(), case_post_command_returns_accepted(), case_command_callback_arrives(), case_locations_emits_x_total_count(), case_locations_limit_truncates_and_links_next(), case_locations_last_page_has_no_link(), case_locations_date_from_filters(), case_locations_date_to_filters(), case_chargingprofiles_list_returns_ok(), case_hubclientinfo_list_returns_ok()]
}
...
fn suite_v230_modules() -> List[cc.Case] {
  [case_chargingprofiles_list_returns_ok(), case_hubclientinfo_list_returns_ok(), case_payments_list_returns_ok(), case_v230_detail_lists_payments()]
}

fn suite_cross_version() -> List[cc.Case] {
  [case_versions_returns_ok(), case_versions_data_is_list(), case_version_detail_returns_ok(), case_locations_list_returns_ok(), case_location_known_has_country_code(), case_location_known_has_evses(), case_location_unknown_returns_2003()]
}

# ---- Entry points ---------------------------------------------
fn default_v221() -> cc.TargetConfig {
  { base_url: "http://localhost:9100/ocpi", token: "cpo-secret", version: "2.2.1" }
}

fn default_v211() -> cc.TargetConfig {
  { base_url: "http://localhost:9100/ocpi", token: "cpo-secret", version: "2.1.1" }
}

fn default_v230() -> cc.TargetConfig {
  { base_url: "http://localhost:9100/ocpi", token: "cpo-secret", version: "2.3.0" }
}

fn run_and_report(banner :: Str, suite :: List[cc.Case], cfg :: cc.TargetConfig) -> [net, io, proc] Int {
  let summary := cc.run_suite(suite, cfg)
  let __lex_discard_1 := io.print(banner)
...
fn main() -> [net, io, proc] Int {
  run_and_report("=== lex-ocpi CPO conformance harness (v2.2.1) ===", suite_v221(), default_v221())
}

fn main_json() -> [net, io, proc] Int {
  let summary := cc.run_suite(suite_v221(), default_v221())
  let __lex_discard_4 := io.print(cc.to_json_str(summary))
  if summary.failed > 0 {
    1 / 0
  } else {
    0
...
fn main_v211() -> [net, io, proc] Int {
  run_and_report("=== lex-ocpi CPO conformance harness (v2.1.1) ===", suite_cross_version(), default_v211())
}

fn main_v230() -> [net, io, proc] Int {
  run_and_report("=== lex-ocpi CPO conformance harness (v2.3.0) ===", list.concat(suite_cross_version(), suite_v230_modules()), default_v230())
}
