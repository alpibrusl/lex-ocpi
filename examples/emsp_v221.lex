...
type CbMsg = CbStore(Str) | CbFetch

type CbReply = CbDone | CbLatest(Option[Str])
...
fn cb_store(actor :: Actor[CbState], body :: Str) -> [concurrent] Unit {
  let __lex_discard_1 := conc.tell(actor, CbStore(body))
  ()
}
...
fn fake_authorize(uid :: Str, _refs :: Option[jv.Json]) -> auth.AuthorizationResult {
...
fn demo_tariff() -> jv.Json {
  JObj([("country_code", JStr(emsp_country())), ("party_id", JStr(emsp_party())), ("id", JStr("TARIFF1")), ("currency", JStr("EUR")), ("elements", JList([JObj([("price_components", JList([JObj([("type", JStr("ENERGY")), ("price", JFloat(0.3)), ("step_size", JInt(1))])]))])])), ("last_updated", JStr("2026-05-15T10:00:00Z"))])
}

# ---- Pure handlers ----------------------------------------------
fn get_versions(_req :: oroute.OcpiRequest) -> oroute.HandlerResult {
  oroute.ok_list([versions.version_to_json(versions.version(versions.v221(), emsp_base_v221()))])
}

fn get_version_detail(_req :: oroute.OcpiRequest) -> oroute.HandlerResult {
  let d := versions.detail(versions.v221(), versions.standard_emsp_v221_endpoints(emsp_base_v221()))
...
fn demo_token() -> jv.Json {
  JObj([("country_code", JStr(emsp_country())), ("party_id", JStr(emsp_party())), ("uid", JStr("RFID-A")), ("type", JStr("RFID")), ("contract_id", JStr("DE-ABC-C12345-T")), ("issuer", JStr("Example eMSP")), ("valid", JBool(true)), ("whitelist", JStr("ALWAYS")), ("last_updated", JStr("2026-05-15T10:00:00Z"))])
}
...
fn registry() -> oroute.Registry {
  (((((oroute.new() |> fn (r :: oroute.Registry) -> oroute.Registry {
    ...
    oroute.handler(r, oroute.post(), "token_authorize", auth221.authorize_handler(fake_authorize))
  }) |> fn (r :: oroute.Registry) -> oroute.Registry {
    oroute.handler(r, oroute.get(), mid.tariffs(), get_tariffs)
  }) |> fn (r :: oroute.Registry) -> oroute.Registry {
    oroute.handler_with_schema(r, oroute.post(), mid.cdrs(), cdrs.validate_cdr, post_cdr)
  }) |> fn (r :: oroute.Registry) -> oroute.Registry {
...
fn handle(actor :: Actor[CbState], req :: Request) -> [time, concurrent] Response {
  ...
  if is_callback_post(req) {
    handle_cb_post(actor, req, timestamp)
  } else {
    if is_callback_get(req) {
      handle_cb_get(actor, timestamp)
    } else {
...
fn handle_cb_post(actor :: Actor[CbState], req :: Request, timestamp :: Str) -> [concurrent] Response {
  let __lex_discard_2 := cb_store(actor, req.body)
  json_response(env.encode(env.ok_empty(timestamp)))
}

fn handle_cb_get(actor :: Actor[CbState], timestamp :: Str) -> [concurrent] Response {
...
fn map_url_to_module(path :: Str) -> Option[RouteHit] {
  if path == "/ocpi/versions" {
    ...
    if path == "/ocpi/2.2.1/" or path == "/ocpi/2.2.1" {
      ...
      if path == "/ocpi/2.2.1/tariffs" {
        Some({ module: mid.tariffs(), params: map.new() })
      } else {
        if path == "/ocpi/2.2.1/cdrs" {
          Some({ module: mid.cdrs(), params: map.new() })
        } else {
          if path == "/ocpi/2.2.1/tokens" {
            Some({ module: mid.tokens(), params: map.new() })
          } else {
            if is_authorize_path(path) {
              let uid := extract_token_uid(path)
              Some({ module: "token_authorize", params: map.set(map.new(), "token_uid", uid) })
            } else {
              None
            }
          }
...
fn is_authorize_path(path :: Str) -> Bool {
  str.starts_with(path, "/ocpi/2.2.1/tokens/") and str.ends_with(path, "/authorize")
}
...
fn ocpi_request(method :: Str, module_name :: Str, path :: Str, params :: Map[Str, Str], req :: Request) -> oroute.OcpiRequest {
...
fn main() -> [net, io, time, concurrent] Nil {
  let __lex_discard_3 := io.print("eMSP v2.2.1  http://localhost:9101/ocpi/versions")
  let actor := conc.spawn({ last: None }, cb_handler)
  net.serve_fn(9101, fn (req :: Request) -> [time, concurrent] Response {
    handle(actor, req)
  })
}
