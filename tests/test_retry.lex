...
fn test_retryable_transport() -> Result[Unit, Str] {
  assert_true(client.is_retryable(HttpFailed("connection refused")), "transport err must retry")
}
...
fn test_retryable_bad_envelope_no() -> Result[Unit, Str] {
  assert_true(client.is_retryable(BadEnvelope("garbage")) == false, "BadEnvelope must NOT retry")
}

fn test_retryable_ocpi_error_no() -> Result[Unit, Str] {
  let resp := { status_code: 2003, status_message: "Unknown Location", data: JNull, timestamp: "2026-05-16T00:00:00Z" }
  assert_true(client.is_retryable(OcpiError(resp)) == false, "OcpiError must NOT retry")
}
...
fn test_retry_after_http_date_unsupported() -> Result[Unit, Str] {
  let h := map.set(map.new(), "retry-after", "Wed, 21 Oct 2026 07:28:00 GMT")
  match client.parse_retry_after_ms(h) {
    None => pass(),
...
# ---- Backoff math (pure) --------------------------------------
fn dflt() -> client.RetryPolicy {
  client.default_retry_policy()
}

fn test_backoff_attempt_1() -> Result[Unit, Str] {
  assert_eq_int(200, client.exp_backoff_ms(1, dflt()), "attempt 1 = base")
}

fn test_backoff_attempt_2() -> Result[Unit, Str] {
  assert_eq_int(400, client.exp_backoff_ms(2, dflt()), "attempt 2 = 2× base")
}

fn test_backoff_attempt_3() -> Result[Unit, Str] {
  assert_eq_int(800, client.exp_backoff_ms(3, dflt()), "attempt 3 = 4× base")
}

# 200 × 2^9 = 102_400 — clamped to max_delay_ms (30_000).
fn test_backoff_caps_at_max() -> Result[Unit, Str] {
  assert_eq_int(30000, client.exp_backoff_ms(10, dflt()), "attempt 10 capped at max")
}

# multiplier_x100 = 100 ⇒ flat delay (no growth).
fn test_backoff_flat_policy() -> Result[Unit, Str] {
  let flat := { max_attempts: 5, initial_delay_ms: 100, max_delay_ms: 30000, multiplier_x100: 100, jitter: false, respect_retry_after: true }
  assert_eq_int(100, client.exp_backoff_ms(5, flat), "flat policy stays at base")
}

# 1.5× multiplier — integer-floor-friendly: 100 → 150 → 225 → 337
fn test_backoff_one_and_half() -> Result[Unit, Str] {
  let p := { max_attempts: 5, initial_delay_ms: 100, max_delay_ms: 30000, multiplier_x100: 150, jitter: false, respect_retry_after: true }
  assert_eq_int(100, client.exp_backoff_ms(1, p), "attempt 1")
  and_ok
  assert_eq_int(150, client.exp_backoff_ms(2, p), "attempt 2")
  and_ok
...
fn test_compute_honours_retry_after() -> [time] Result[Unit, Str] {
  let p := { max_attempts: 5, initial_delay_ms: 200, max_delay_ms: 30000, multiplier_x100: 200, jitter: false, respect_retry_after: true }
...
fn test_compute_caps_retry_after() -> [time] Result[Unit, Str] {
  let p := { max_attempts: 5, initial_delay_ms: 200, max_delay_ms: 1000, multiplier_x100: 200, jitter: false, respect_retry_after: true }
...
fn test_compute_ignores_retry_after_when_off() -> [time] Result[Unit, Str] {
  let p := { max_attempts: 5, initial_delay_ms: 200, max_delay_ms: 30000, multiplier_x100: 200, jitter: false, respect_retry_after: false }
  let got := client.compute_backoff_ms(1, p, Some(5000))
...
fn test_compute_jitter_in_range() -> [time] Result[Unit, Str] {
  let p := { max_attempts: 5, initial_delay_ms: 1000, max_delay_ms: 30000, multiplier_x100: 100, jitter: true, respect_retry_after: true }
  let got := client.compute_backoff_ms(1, p, None)
  if got >= 800 and got <= 1200 {
    pass()
  } else {
    fail(str.concat("jittered value out of [800,1200]: ", int.to_str(got)))
  }
}

# ---- reason_of -------------------------------------------------
fn test_reason_transport() -> Result[Unit, Str] {
  assert_eq_str("transport: refused", client.reason_of(HttpFailed("refused")), "transport reason")
}
...
fn test_reason_bad_envelope() -> Result[Unit, Str] {
  assert_eq_str("bad-envelope: nope", client.reason_of(BadEnvelope("nope")), "bad envelope reason")
}

fn test_reason_ocpi_error() -> Result[Unit, Str] {
  let resp := { status_code: 2003, status_message: "Unknown Location", data: JNull, timestamp: "2026-05-16T00:00:00Z" }
  assert_eq_str("ocpi-error", client.reason_of(OcpiError(resp)), "ocpi reason")
}

# ---- Policy constructors --------------------------------------
fn test_default_policy_shape() -> Result[Unit, Str] {
  let p := client.default_retry_policy()
  if p.max_attempts == 5 and p.initial_delay_ms == 200 and p.max_delay_ms == 30000 and p.multiplier_x100 == 200 and p.jitter == true and p.respect_retry_after == true {
    pass()
  } else {
...
fn pure_suite() -> List[Result[Unit, Str]] {
  [test_retryable_transport(), test_retryable_http_503(), test_retryable_http_429(), test_retryable_http_408(), test_retryable_http_404_no(), test_retryable_http_401_no(), test_retryable_bad_envelope_no(), test_retryable_ocpi_error_no(), test_status_500(), test_status_599(), test_status_600_no(), test_status_499_no(), test_status_200_no(), test_retry_after_integer_seconds(), test_retry_after_zero(), test_retry_after_missing(), test_retry_after_negative(), test_retry_after_garbage(), test_retry_after_http_date_unsupported(), test_hint_on_503(), test_hint_on_429(), test_hint_not_on_500(), test_hint_not_on_transport(), test_backoff_attempt_1(), test_backoff_attempt_2(), test_backoff_attempt_3(), test_backoff_caps_at_max(), test_backoff_flat_policy(), test_backoff_one_and_half(), test_reason_transport(), test_reason_http_status(), test_reason_bad_envelope(), test_reason_ocpi_error(), test_default_policy_shape(), test_no_retry_policy_shape()]
}

fn time_suite() -> [time] List[Result[Unit, Str]] {
  [test_compute_honours_retry_after(), test_compute_caps_retry_after(), test_compute_ignores_retry_after_when_off(), test_compute_no_hint_uses_exp(), test_compute_jitter_in_range()]
}
