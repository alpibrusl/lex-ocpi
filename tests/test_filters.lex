...
fn test_from_query_both() -> Result[Unit, Str] {
  let q := map.set(map.set(map.new(), "date_from", "2026-05-01T00:00:00Z"), "date_to", "2026-06-01T00:00:00Z")
  let r := f.from_query(q)
  match r.date_from {
    None => fail("date_from missing"),
    Some(s) => if s == "2026-05-01T00:00:00Z" {
      pass()
    } else {
      fail("date_from value wrong")
    },
  }
}
...
fn test_apply_lower_only() -> Result[Unit, Str] {
  ...
  assert_eq_int(2, list.len(kept), "lower bound 2026-05-01 should keep 2 of 3")
}
...
fn test_apply_upper_only() -> Result[Unit, Str] {
  ...
  let kept := f.apply(items, r)
  assert_eq_int(2, list.len(kept), "upper bound 2026-06-01 (exclusive) should keep 2 of 3")
}

fn test_apply_both_bounds() -> Result[Unit, Str] {
  let items := [item("2026-04-01"), item("2026-05-15"), item("2026-06-01"), item("2026-07-01")]
  let r := { date_from: Some("2026-05-01"), date_to: Some("2026-07-01") }
  let kept := f.apply(items, r)
  assert_eq_int(2, list.len(kept), "[05-01, 07-01) keeps 2 of 4")
}

fn test_apply_no_last_updated() -> Result[Unit, Str] {
  let items := [JObj([("id", JStr("no-timestamp"))]), item("2026-05-15")]
  let kept := f.apply(items, f.any())
  assert_eq_int(1, list.len(kept), "items lacking last_updated should be dropped")
}
...
fn suite() -> List[Result[Unit, Str]] {
  [test_from_query_empty(), test_from_query_both(), test_apply_no_bounds(), test_apply_lower_only(), test_apply_upper_only(), test_apply_both_bounds(), test_apply_no_last_updated(), test_str_lt_basic(), test_str_ge_equal(), test_str_ge_different_length()]
}
