...
fn test_client_error_codes() -> Result[Unit, Str] {
  if status.invalid_or_missing_parameters() == 2001 and status.not_enough_information() == 2002 and status.unknown_location() == 2003 and status.unknown_token() == 2004 {
    pass()
  } else {
...
fn test_server_error_codes() -> Result[Unit, Str] {
  if status.server_error() == 3000 and status.unable_to_use_api() == 3001 and status.unsupported_version() == 3002 and status.no_matching_endpoints() == 3003 {
    pass()
  } else {
...
fn test_hub_error_codes() -> Result[Unit, Str] {
  if status.hub_error() == 4000 and status.missing_or_invalid_parameters() == 4001 and status.unknown_receiver() == 4002 and status.timeout_on_forwarded_request() == 4003 and status.connection_problem() == 4004 {
    pass()
  } else {
...
fn test_predicates() -> Result[Unit, Str] {
  if status.is_success(1000) and status.is_client_error(2001) and status.is_server_error(3000) and status.is_hub_error(4001) {
    pass()
  } else {
...
fn test_to_message_known() -> Result[Unit, Str] {
  if str.is_empty(status.to_message(1000)) or str.is_empty(status.to_message(2003)) {
...
fn test_all_codes_unique_and_nonempty() -> Result[Unit, Str] {
  ...
  if list.len(codes) >= 15 {
    pass()
  } else {
    fail("all_codes should list at least 15 entries")
  }
}

# ---- Suite + runner ---------------------------------------------
fn suite() -> List[Result[Unit, Str]] {
  [test_success_code(), test_client_error_codes(), test_server_error_codes(), test_hub_error_codes(), test_predicates(), test_to_message_known(), test_to_message_unknown(), test_all_codes_unique_and_nonempty()]
}
