...
fn round_trip(label :: Str, schema :: sch.ModelSchema, validator :: (jv.Json) -> Result[jv.Json, List[{ path :: Str, code :: Str, message :: Str }]], seed :: Int) -> [random] Result[Unit, Str] {
  ...
  match validator(v) {
    Ok(_) => pass(),
    Err(es) => fail(str.concat(label, str.concat(": generated payload failed validation: ", str.concat(jv.stringify(v), str.concat(" — errors: ", int_count(es)))))),
  }
}
...
fn test_connector_property() -> [random] Result[Unit, Str] {
  round_trip("Connector", locs.connector_schema(), locs.validate_connector, 11)
}

fn test_evse_property() -> [random] Result[Unit, Str] {
  round_trip("EVSE", locs.evse_schema(), locs.validate_evse, 12)
}

fn test_location_property() -> [random] Result[Unit, Str] {
  round_trip("Location", locs.location_schema(), locs.validate_location, 13)
}

fn test_session_property() -> [random] Result[Unit, Str] {
  round_trip("Session", sess.session_schema(), sess.validate_session, 14)
}

fn test_token_property() -> [random] Result[Unit, Str] {
  round_trip("Token", tokens.token_schema(), tokens.validate_token, 15)
}

fn test_credentials_property() -> [random] Result[Unit, Str] {
  round_trip("Credentials", creds.credentials_schema_v221(), creds.validate_credentials_v221, 16)
}

fn test_charging_profile_property() -> [random] Result[Unit, Str] {
  round_trip("ChargingProfile", cp.charging_profile_schema(), cp.validate_charging_profile, 17)
}

fn test_client_info_property() -> [random] Result[Unit, Str] {
  round_trip("ClientInfo", hub.client_info_schema(), hub.validate_client_info, 18)
}

# ---- Suite + runner ---------------------------------------------
fn suite() -> [random] List[Result[Unit, Str]] {
  [test_connector_property(), test_evse_property(), test_location_property(), test_session_property(), test_token_property(), test_credentials_property(), test_charging_profile_property(), test_client_info_property()]
}
