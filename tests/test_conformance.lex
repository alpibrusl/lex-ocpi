...
fn test_classify_success() -> Result[Unit, Str] {
  match conf.classify(1000) {
    Success => pass(),
    _ => fail("1000 should be Success"),
  }
}

fn test_classify_client() -> Result[Unit, Str] {
  match conf.classify(2003) {
    ClientError => pass(),
    _ => fail("2003 should be ClientError"),
  }
}

fn test_classify_server() -> Result[Unit, Str] {
  match conf.classify(3001) {
    ServerError => pass(),
    _ => fail("3001 should be ServerError"),
  }
}

fn test_classify_hub() -> Result[Unit, Str] {
  match conf.classify(4001) {
    HubError => pass(),
    _ => fail("4001 should be HubError"),
  }
}

fn test_classify_unknown_low() -> Result[Unit, Str] {
  match conf.classify(500) {
    Unknown => pass(),
    _ => fail("500 should be Unknown"),
  }
}

fn test_classify_unknown_high() -> Result[Unit, Str] {
  match conf.classify(5000) {
    Unknown => pass(),
    _ => fail("5000 should be Unknown"),
  }
}

# ---- check_envelope --------------------------------------------
fn ok_envelope() -> env.OcpiResponse {
  { data: JNull, status_code: 1000, status_message: "", timestamp: "2026-05-16T00:00:00Z" }
}

fn err_envelope(code :: Int) -> env.OcpiResponse {
  { data: JNull, status_code: code, status_message: "Bad", timestamp: "2026-05-16T00:00:00Z" }
}
...
fn test_envelope_missing_timestamp() -> Result[Unit, Str] {
  let r := { data: JNull, status_code: 1000, status_message: "", timestamp: "" }
  assert_err(conf.check_envelope(r), "empty timestamp")
}

fn test_envelope_unknown_band() -> Result[Unit, Str] {
  let r := { data: JNull, status_code: 9999, status_message: "x", timestamp: "2026-05-16T00:00:00Z" }
  assert_err(conf.check_envelope(r), "9999 status_code")
}

fn test_envelope_message_missing_on_err() -> Result[Unit, Str] {
  let r := { data: JNull, status_code: 2003, status_message: "", timestamp: "2026-05-16T00:00:00Z" }
...
fn test_envelope_all_collects_errors() -> Result[Unit, Str] {
  let r := { data: JNull, status_code: 2003, status_message: "", timestamp: "" }
  let errs := conf.check_envelope_all(r)
  assert_true(list.len(errs) == 2, str.concat("expected 2 errors, got ", int.to_str(list.len(errs))))
}

# ---- check_module_request_headers ----------------------------
fn good_headers() -> h.OcpiHeaders {
  h.new("Token abc", "rid-1", "corr-1", party.new("NL", "EXM"), party.new("DE", "ABC"))
}

fn test_module_headers_happy() -> Result[Unit, Str] {
  assert_ok(conf.check_module_request_headers(good_headers()), "well-formed module headers")
}

fn test_module_headers_missing_auth() -> Result[Unit, Str] {
  let hs := h.new("", "rid-1", "corr-1", party.new("NL", "EXM"), party.new("DE", "ABC"))
...
fn test_module_headers_wrong_auth_scheme() -> Result[Unit, Str] {
  let hs := h.new("Bearer xyz", "rid-1", "corr-1", party.new("NL", "EXM"), party.new("DE", "ABC"))
  assert_err(conf.check_module_request_headers(hs), "Bearer rejected")
}

fn test_module_headers_missing_request_id() -> Result[Unit, Str] {
  let hs := h.new("Token x", "", "corr", party.new("NL", "EXM"), party.new("DE", "ABC"))
...
fn test_module_headers_missing_from_party() -> Result[Unit, Str] {
  let hs := h.new("Token x", "rid", "corr", party.new("", ""), party.new("DE", "ABC"))
...
fn test_module_headers_country_wrong_length() -> Result[Unit, Str] {
  let hs := h.new("Token x", "rid", "corr", party.new("NLD", "EXM"), party.new("DE", "ABC"))
...
fn test_echo_happy() -> Result[Unit, Str] {
  ...
  let resp := map.set(map.set(map.new(), "x-request-id", "rid-1"), "x-correlation-id", "corr-1")
...
fn test_echo_request_id_missing() -> Result[Unit, Str] {
  ...
  assert_err(conf.check_response_echoes_request(req, resp), "missing request_id in resp")
}

fn test_echo_request_id_mismatch() -> Result[Unit, Str] {
  let req := good_headers()
  let resp := map.set(map.set(map.new(), "x-request-id", "rid-DIFFERENT"), "x-correlation-id", "corr-1")
  assert_err(conf.check_response_echoes_request(req, resp), "request_id changed in resp")
}

# ---- check_pagination_headers ---------------------------------
fn test_pagination_happy() -> Result[Unit, Str] {
  let resp := map.set(map.set(map.new(), "x-total-count", "42"), "x-limit", "10")
...
fn test_auth_token_scheme() -> Result[Unit, Str] {
  let hs := h.new("Token base64payload", "rid", "corr", party.new("NL", "EXM"), party.new("DE", "ABC"))
...
fn mk_request(path :: Str) -> route.OcpiRequest {
  route.request("GET", "locations", path, map.new(), map.new(), good_headers(), JNull)
}

fn test_flow_dispatch_envelope_conforms() -> Result[Unit, Str] {
  let resp := route.dispatch(build_registry(), mk_request("/locations"), "2026-05-16T00:00:00Z")
  match conf.check_envelope(resp) {
    Ok(_) => pass(),
    Err(e) => fail(str.concat("dispatched response not spec-conforming: ", conf.render(e))),
  }
}

fn test_flow_unknown_route_envelope_conforms() -> Result[Unit, Str] {
  let resp := route.dispatch(route.new(), mk_request("/locations"), "2026-05-16T00:00:00Z")
  let env_ok := match conf.check_envelope(resp) {
    Ok(_) => pass(),
    Err(e) => fail(str.concat("fallback envelope not conforming: ", conf.render(e))),
  }
  let band_ok := match conf.classify(resp.status_code) {
    ClientError => pass(),
    _ => fail(str.concat("fallback should be 2xxx, got ", int.to_str(resp.status_code))),
  }
  match env_ok {
    Err(_) => env_ok,
    Ok(_) => band_ok,
