...
fn test_from_map_full() -> Result[Unit, Str] {
  ...
  if h.authorization == "Token abc==" and h.request_id == "req-1" and h.from_party.country_code == "NL" and h.to_party.party_id == "BMW" {
    pass()
  } else {
...
fn test_from_map_partial() -> Result[Unit, Str] {
  ...
  if h.authorization == "Token xyz" and h.request_id == "" and str.is_empty(h.from_party.country_code) {
    pass()
  } else {
...
fn test_has_party_routing_true() -> Result[Unit, Str] {
  let h := headers.from_map(full_map())
  assert_true(headers.has_party_routing(h), "full quad should be considered routed")
}

fn test_has_party_routing_false() -> Result[Unit, Str] {
  let h := headers.from_map(map.new())
  assert_true(not headers.has_party_routing(h), "empty quad should not be considered routed")
}
...
fn test_round_trip_to_map() -> Result[Unit, Str] {
  ...
  if h.authorization == h2.authorization and headers.has_party_routing(h2) {
    pass()
  } else {
...
fn suite() -> List[Result[Unit, Str]] {
  [test_from_map_full(), test_from_map_partial(), test_strip_token(), test_strip_token_bad_prefix(), test_strip_token_empty(), test_has_party_routing_true(), test_has_party_routing_false(), test_round_trip_to_map()]
}
