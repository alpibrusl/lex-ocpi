...
fn mk_headers(rid :: Str, cc :: Str, pid :: Str) -> h.OcpiHeaders {
  h.new("Token x", rid, "corr-1",
        party.new(cc, pid),
        party.new("DE", "ABC"))   # to-party irrelevant for caching
}
...
fn test_key_from_request_happy() -> Result[Unit, Str] {
  ...
  match idem.key_from_request(req) {
    ...
    Some(k) => {
      assert_eq_str("POST|/ocpi/2.2.1/cdrs|req-123|NL|EXM",
                    idem.key_str(k), "key serialisation")
    },
  }
}

fn test_key_from_request_missing_id() -> Result[Unit, Str] {
  # Empty request_id → uncacheable.
...
fn test_handler_reserve_then_hit() -> Result[Unit, Str] {
  ...
  # First reserve → Run; the slot is now InFlight.
  let s2 := fst_after(s1, Store("k1", mk_response(1000, "OK"), 999_999_999_999))
  # Subsequent reserve on the SAME key → Hit with the stored response.
...
fn test_handler_reserve_inflight_wait() -> Result[Unit, Str] {
  ...
  # Another TryReserve while still InFlight → Wait.
...
fn test_handler_lookup_completed_hit() -> Result[Unit, Str] {
  ...
  let s2 := fst_after(s1, Store("k5", mk_response(1000, "OK"), 999_999_999_999))
...
fn test_handler_lookup_expired_miss() -> Result[Unit, Str] {
  ...
  # expires_at_ns = 50; now_ns = 100 → expired.
...
fn test_handler_forget_clears_inflight() -> Result[Unit, Str] {
  ...
  # After Forget, a fresh TryReserve should get Run again.
...
fn test_handler_purge_drops_expired() -> Result[Unit, Str] {
  ...
  let s2 := fst_after(s1, Store("k8", mk_response(1000, "OK"), 50))   # expires at 50
  let s3 := fst_after(s2, TryReserve("k9"))
  let s4 := fst_after(s3, Store("k9", mk_response(1000, "OK"), 200))  # expires at 200
  # Purge at now=100 → drops k8, keeps k9.
...
fn test_lru_evicts_oldest() -> Result[Unit, Str] {
  let cap := 3
  let s1 := fst_after(empty(cap),
              Store("a", mk_response(1000, "OK"), 999_999_999_999))
  let s2 := fst_after(s1,
              Store("b", mk_response(1000, "OK"), 999_999_999_999))
  let s3 := fst_after(s2,
              Store("c", mk_response(1000, "OK"), 999_999_999_999))
  # At capacity (3). Insert d → evicts the oldest (a).
  let s4 := fst_after(s3,
              Store("d", mk_response(1000, "OK"), 999_999_999_999))
...
fn test_lru_touch_promotes() -> Result[Unit, Str] {
  ...
  let s3 := fst_after(s2,
              Store("c", mk_response(1000, "OK"), 999_999_999_999))
  # Touch "a" by re-reserving — bumps it to head.
  let s4 := fst_after(s3, TryReserve("a"))
  # Now b is the LRU tail. Insert d → b evicted, not a.
  let s5 := fst_after(s4,
              Store("d", mk_response(1000, "OK"), 999_999_999_999))
...
fn test_lru_capacity_zero_unbounded() -> Result[Unit, Str] {
  # capacity <= 0 → no eviction. Insert 5 entries, all survive.
  let s := list.fold(["a","b","c","d","e"], empty(0),
    fn (acc :: idem.CacheState, k :: Str) -> idem.CacheState {
      fst_after(acc, Store(k, mk_response(1000, "OK"), 999_999_999_999))
    })
...
fn test_actor_basic_flow() -> [concurrent] Result[Unit, Str] {
  ...
  let _ := idem.store_response(actor, "ak1", mk_response(1000, "OK"),
                                999_999_999_999)
  # Second call → Hit.
...
fn test_actor_forget_releases() -> [concurrent] Result[Unit, Str] {
  let actor := idem.new_cache(10)
  let _ := idem.try_reserve(actor, "ak2")
  let _ := idem.forget(actor, "ak2")
...
fn test_actor_lookup_pending() -> [concurrent] Result[Unit, Str] {
  let actor := idem.new_cache(10)
  let _ := idem.try_reserve(actor, "ak3")
...
fn test_dispatch_dedups_same_key() -> [concurrent, time] Result[Unit, Str] {
  ...
  # First call: 1000-payload handler. Cache stores response.
  let _ := idem.dispatch_with_cache(registry_1000(), cache, cfg, req, "t")
  # Second call with SAME key but a 2000-payload handler. Cache
  # should serve the first response → we still see "1000".
...
fn test_dispatch_distinct_keys_both_run() -> [concurrent, time] Result[Unit, Str] {
  let cache := idem.new_cache(100)
  let cfg   := idem.default_config()
  # Different keys → each call runs its own handler → each
  # response reflects the registry it was given.
  let r1 := idem.dispatch_with_cache(registry_1000(), cache, cfg,
              mk_request("POST", "/cdrs", "x-1"), "t")
  let r2 := idem.dispatch_with_cache(registry_2000(), cache, cfg,
              mk_request("POST", "/cdrs", "x-2"), "t")
...
fn test_dispatch_no_request_id_skips_cache() -> [concurrent, time] Result[Unit, Str] {
  let cache := idem.new_cache(100)
  let cfg   := idem.default_config()
  let req   := mk_request("POST", "/cdrs", "")  # empty request_id → uncacheable
  # Both calls bypass the cache and each registry runs its own handler.
...
fn test_dispatch_inflight_fallback_after_timeout()
  -> [concurrent, time] Result[Unit, Str] {
  let cache := idem.new_cache(100)
  let cfg := { ttl_ms: 24 * 60 * 60 * 1000,
               poll_interval_ms: 5,
               max_wait_ms: 20 }
  let req := mk_request("POST", "/cdrs", "stuck-1")
  let key := idem.key_str({
    method: "POST", path: "/cdrs", request_id: "stuck-1",
    from_cc: "NL", from_party_id: "EXM" })
  # Stuff an InFlight marker so the first try_reserve sees Wait.
  let _ := idem.try_reserve(cache, key)
  # Caller polls, times out (~20ms), clears the marker, runs the handler.
...
fn pure_suite() -> List[Result[Unit, Str]] {
  [
    # key derivation
    test_key_from_request_happy(),
    test_key_from_request_missing_id(),
    # pure handler basics
    test_handler_first_reserve_run(),
    test_handler_reserve_then_hit(),
    test_handler_reserve_inflight_wait(),
    test_handler_lookup_miss(),
    test_handler_lookup_inflight_yields_wait(),
    test_handler_lookup_completed_hit(),
    test_handler_lookup_expired_miss(),
    test_handler_forget_clears_inflight(),
    # purge
    test_handler_purge_drops_expired(),
    # LRU
    test_lru_evicts_oldest(),
    test_lru_touch_promotes(),
    test_lru_capacity_zero_unbounded(),
  ]
}

fn actor_suite() -> [concurrent] List[Result[Unit, Str]] {
  [
    test_actor_basic_flow(),
    test_actor_forget_releases(),
    test_actor_lookup_pending(),
  ]
}

fn dispatch_suite() -> [concurrent, time] List[Result[Unit, Str]] {
  [
    test_dispatch_dedups_same_key(),
    test_dispatch_distinct_keys_both_run(),
    test_dispatch_no_request_id_skips_cache(),
    test_dispatch_inflight_fallback_after_timeout(),
  ]
}
...
fn run_all() -> [concurrent, time] Int {
  count_failures(pure_suite())
    + count_failures(actor_suite())
    + count_failures(dispatch_suite())
}
