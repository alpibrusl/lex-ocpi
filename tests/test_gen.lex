...
fn test_minimal_schema() -> Result[Unit, Str] {
  ...
  match gen.generate(input) {
    ...
    Ok(out) => assert_contains(out, "fn heartbeat_schema", "schema fn name"),
  }
}
...
fn test_required_vs_optional() -> Result[Unit, Str] {
  ...
  match gen.generate(input) {
    Err(e) => fail(str.concat("generate failed: ", e)),
    Ok(out) => if str.contains(out, "s.required_str(\"a\"") and str.contains(out, "s.optional(s.required_str(\"b\"") {
      pass()
    } else {
...
fn test_validator_emitted() -> Result[Unit, Str] {
  ...
  match gen.generate(input) {
    Err(e) => fail(str.concat("generate failed: ", e)),
    Ok(out) => assert_contains(out, "fn validate_heartbeat(j :: jv.Json)", "validator wrapper"),
  }
}
...
fn test_camel_to_snake() -> Result[Unit, Str] {
  ...
  match gen.generate(input) {
    Err(e) => fail(str.concat("generate failed: ", e)),
    Ok(out) => assert_contains(out, "fn boot_notification_request_schema", "snake_case name"),
  }
}
...
fn suite() -> List[Result[Unit, Str]] {
  [test_minimal_schema(), test_required_vs_optional(), test_enum(), test_max_len(), test_int_min_zero(), test_int_min_one(), test_array_of_strings(), test_validator_emitted(), test_camel_to_snake(), test_bad_input()]
}
