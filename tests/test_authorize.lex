...
fn test_url_v221() -> Result[Unit, Str] {
  assert_eq_str("https://emsp.example/ocpi/2.2.1/tokens/NL/TNM/RFID-A/authorize", auth221.build_authorize_url("https://emsp.example/ocpi/2.2.1/tokens", "NL", "TNM", "RFID-A"), "v2.2.1 url")
}

fn test_url_v211() -> Result[Unit, Str] {
  assert_eq_str("https://emsp.example/ocpi/2.1.1/tokens/RFID-A/authorize", auth211.build_authorize_url("https://emsp.example/ocpi/2.1.1/tokens", "RFID-A"), "v2.1.1 url")
}

fn test_url_v230() -> Result[Unit, Str] {
  assert_eq_str("https://emsp.example/ocpi/2.3.0/tokens/DE/BMW/RFID-Z/authorize", auth230.build_authorize_url("https://emsp.example/ocpi/2.3.0/tokens", "DE", "BMW", "RFID-Z"), "v2.3.0 url")
}
...
fn test_body_some_stringifies() -> Result[Unit, Str] {
  let refs := JObj([("location_id", JStr("LOC1"))])
...
fn mk_request(token_uid :: Str, body :: jv.Json) -> route.OcpiRequest {
  route.request(route.post(), "tokens", "/ocpi/2.2.1/tokens/NL/TNM/RFID-A/authorize", map.set(map.new(), "token_uid", token_uid), map.new(), empty_headers(), body)
}

# Always-Allowed authorisation function — exercises the happy path.
fn auth_always_allowed(_uid :: Str, _refs :: Option[jv.Json]) -> auth.AuthorizationResult {
  Allowed(JObj([("allowed", JStr("ALLOWED"))]))
}

fn auth_always_blocked(_uid :: Str, _refs :: Option[jv.Json]) -> auth.AuthorizationResult {
...
fn suite() -> List[Result[Unit, Str]] {
  [test_decode_allowed(), test_decode_blocked(), test_decode_expired(), test_decode_no_credit(), test_decode_not_allowed(), test_decode_missing_allowed(), test_decode_non_string_allowed(), test_decode_unknown_allowed(), test_round_trip_allowed(), test_round_trip_not_allowed(), test_url_v221(), test_url_v211(), test_url_v230(), test_body_none_is_empty_object(), test_body_some_stringifies(), test_handler_allowed_branch(), test_handler_blocked_branch(), test_handler_missing_token_uid(), test_round_trip_handler_to_decoder(), test_body_validator_accepts_null(), test_body_validator_accepts_empty_object(), test_body_validator_accepts_valid_refs(), test_body_validator_rejects_missing_location_id()]
}
