...
fn empty_state() -> Map[Str, Option[cmds.CommandResult]] { map.new() }
...
fn test_handler_complete_then_lookup() -> Result[Unit, Str] {
  ...
  let s2 := tuple.fst(ca.inflight_handler(s1,
              Complete("R3", cmds.result_accepted())))
...
fn test_handler_complete_without_register() -> Result[Unit, Str] {
  let s1 := tuple.fst(ca.inflight_handler(empty_state(),
              Complete("R-orphan", cmds.result_timeout())))
...
fn test_handler_forget() -> Result[Unit, Str] {
  ...
  let s2 := tuple.fst(ca.inflight_handler(s1,
              Complete("R4", cmds.result_accepted())))
  let step := ca.inflight_handler(s2, Forget("R4"))
  match tuple.snd(step) {
    AckForgotten => assert_true(map.has(tuple.fst(step), "R4") == false,
                                "R4 still present after Forget"),
...
fn test_actor_register_then_complete() -> [concurrent] Result[Unit, Str] {
  ...
  let _ := ca.register(actor, "A-1")
  let _ := ca.complete(actor, "A-1", cmds.result_accepted())
...
fn test_actor_forget() -> [concurrent] Result[Unit, Str] {
  let actor := ca.new_inflight()
  let _ := ca.register(actor, "A-3")
  let _ := ca.complete(actor, "A-3", cmds.result_failed())
  let _ := ca.forget(actor, "A-3")
...
fn test_compute_deadline_positive() -> Result[Unit, Str] {
  # 1500ms == 1_500_000_000ns.
  assert_true(ca.compute_deadline_from(0, 1500) == 1_500_000_000,
              "1500ms -> 1.5e9 ns")
}

fn test_compute_deadline_offset_from_start() -> Result[Unit, Str] {
  let start := 5_000_000_000
  assert_true(ca.compute_deadline_from(start, 250) == 5_250_000_000,
              "start + 250ms")
}
...
fn test_wait_returns_completed() -> [concurrent, time] Result[Unit, Str] {
  let actor := ca.new_inflight()
  let _ := ca.register(actor, "W-1")
  let _ := ca.complete(actor, "W-1", cmds.result_accepted())
  match ca.wait_for_result(actor, "W-1", 1_000, 10) {
    Ok(r) => match r.result {
      ResAccepted => pass(),
...
fn test_wait_unknown_id() -> [concurrent, time] Result[Unit, Str] {
  let actor := ca.new_inflight()
  match ca.wait_for_result(actor, "W-never", 1_000, 10) {
...
fn test_parse_bad_response_id() -> Result[Unit, Str] {
  let body := JObj([("result", JStr("ACCEPTED"))])
  # Extractor that always returns None — simulates a route the
  # eMSP forgot to wire a path param for.
  let extractor := fn (_ :: route.OcpiRequest) -> Option[Str] { None }
...
fn pure_suite() -> List[Result[Unit, Str]] {
  [
    ...
    test_handler_lookup_unknown(),
    test_handler_lookup_pending(),
    test_handler_complete_then_lookup(),
    test_handler_complete_without_register(),
    test_handler_forget(),
    # Deadline math
    test_compute_deadline_zero(),
    test_compute_deadline_positive(),
    test_compute_deadline_offset_from_start(),
    # Parser
    test_parse_completed(),
    test_parse_bad_response_id(),
    test_parse_bad_body_missing_result(),
    test_parse_bad_body_unknown_result(),
    # Handler-result mapping
    test_handler_result_ok(),
    test_handler_result_bad_id(),
...
fn actor_suite() -> [concurrent] List[Result[Unit, Str]] {
  [
    test_actor_register_then_complete(),
    test_actor_unknown_id(),
    test_actor_pending(),
    test_actor_forget(),
  ]
}
...
fn wait_suite() -> [concurrent, time] List[Result[Unit, Str]] {
  [
    test_wait_returns_completed(),
    test_wait_unknown_id(),
    test_wait_timeout(),
  ]
}
