...
fn test_result_type_accepted() -> Result[Unit, Str] {
  match cmds.decode_command_result_type("ACCEPTED") {
    ...
    Ok(t) => assert_eq_str("ACCEPTED", cmds.encode_command_result_type(t), "ResAccepted"),
  }
}
...
fn test_result_type_canceled_reservation() -> Result[Unit, Str] {
  match cmds.decode_command_result_type("CANCELED_RESERVATION") {
    Err(m) => fail(str.concat("decode failed: ", m)),
    Ok(t) => assert_eq_str("CANCELED_RESERVATION", cmds.encode_command_result_type(t), "ResCanceledReservation"),
  }
}
...
fn test_result_type_timeout() -> Result[Unit, Str] {
  match cmds.decode_command_result_type("TIMEOUT") {
    Err(m) => fail(str.concat("decode failed: ", m)),
    Ok(t) => assert_eq_str("TIMEOUT", cmds.encode_command_result_type(t), "ResTimeout"),
  }
}
...
fn test_response_encode_minimal() -> Result[Unit, Str] {
  ...
  assert_eq_str("{\"result\":\"ACCEPTED\"}", jv.stringify(j), "minimal response encoding")
}

fn test_response_encode_with_timeout() -> Result[Unit, Str] {
  let r := cmds.response(CrAccepted, Some(30), [])
  let j := cmds.encode_command_response(r)
  assert_eq_str("{\"result\":\"ACCEPTED\",\"timeout\":30}", jv.stringify(j), "response w/ timeout encoding")
}
...
fn test_response_decode_round_trip() -> Result[Unit, Str] {
  ...
  match cmds.decode_command_response(j) {
    ...
    Ok(r2) => {
      ...
      assert_eq_str(jv.stringify(j), jv.stringify(j2), "response round-trip")
    },
  }
}
...
fn test_result_encode_minimal() -> Result[Unit, Str] {
  ...
  assert_eq_str("{\"result\":\"ACCEPTED\"}", jv.stringify(j), "minimal result encoding")
}
...
fn test_result_decode_round_trip() -> Result[Unit, Str] {
  ...
  match cmds.decode_command_result(j) {
    ...
    Ok(r2) => {
      ...
      assert_eq_str(jv.stringify(j), jv.stringify(j2), "result round-trip")
    },
  }
}
...
fn test_response_url_present() -> Result[Unit, Str] {
  let body := JObj([("response_url", JStr("https://emsp.example/callback/abc123")), ("session_id", JStr("S1"))])
  match cmds.response_url(body) {
    None => fail("expected Some(response_url)"),
    Some(u) => assert_eq_str("https://emsp.example/callback/abc123", u, "response_url"),
  }
}
...
fn test_url_start_session() -> Result[Unit, Str] {
  assert_eq_str("https://cpo.example/ocpi/2.2.1/commands/START_SESSION", cmds.build_command_url("https://cpo.example/ocpi/2.2.1/commands", StartSession), "start_session url")
}

fn test_url_unlock_connector() -> Result[Unit, Str] {
  assert_eq_str("https://cpo.example/ocpi/2.3.0/commands/UNLOCK_CONNECTOR", cmds.build_command_url("https://cpo.example/ocpi/2.3.0/commands", UnlockConnector), "unlock_connector url")
}

fn test_url_cancel_reservation() -> Result[Unit, Str] {
  assert_eq_str("https://cpo.example/ocpi/2.2.1/commands/CANCEL_RESERVATION", cmds.build_command_url("https://cpo.example/ocpi/2.2.1/commands", CancelReservation), "cancel_reservation url")
}
...
fn mk_request(body :: jv.Json) -> route.OcpiRequest {
  route.request(route.post(), "commands.START_SESSION", "/ocpi/2.2.1/commands/START_SESSION", map.new(), map.new(), empty_headers(), body)
}
...
fn check_captured_url(items :: List[jv.Json]) -> Result[Unit, Str] {
  match list.head(items) {
    ...
    Some(h) => match jv.as_str(h) {
      ...
      Some(s) => assert_eq_str("https://emsp.example/cb/xyz", s, "handler captured wrong url"),
    },
  }
}
