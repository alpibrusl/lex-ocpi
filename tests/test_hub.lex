...
fn cpo_a() -> party.PartyId { party.new("NL", "EXM") }
fn cpo_b() -> party.PartyId { party.new("NL", "OTH") }
fn emsp_1() -> party.PartyId { party.new("DE", "ABC") }
fn emsp_2() -> party.PartyId { party.new("FR", "XYZ") }
fn hub_party() -> party.PartyId { party.new("NL", "HUB") }
...
fn three_peer_table() -> hub.RoutingTable {
  ...
  let t1 := hub.add_peer(t0,
    target_for(cpo_a(), "https://cpo-a.example/ocpi/2.2.1", "tok-a"))
  let t2 := hub.add_peer(t1,
    target_for(emsp_1(), "https://emsp-1.example/ocpi/2.2.1", "tok-1"))
  hub.add_peer(t2,
    target_for(emsp_2(), "https://emsp-2.example/ocpi/2.2.1", "tok-2"))
}
...
fn test_add_then_count() -> Result[Unit, Str] {
  let t := hub.add_peer(hub.empty_table(),
    target_for(cpo_a(), "https://x", "tok"))
...
fn test_lookup_hit() -> Result[Unit, Str] {
  match hub.lookup(three_peer_table(), emsp_1()) {
    Some(t) => assert_eq_str("https://emsp-1.example/ocpi/2.2.1",
                              t.base_url, "lookup hit base_url"),
...
fn test_remove_unknown_is_noop() -> Result[Unit, Str] {
  ...
  assert_eq_int(hub.peer_count(t), hub.peer_count(t2),
                "remove unknown changes nothing")
}

fn test_add_overwrites() -> Result[Unit, Str] {
  let t  := three_peer_table()
  # Re-add emsp_1 with a new base_url + token. Should overwrite.
  let t2 := hub.add_peer(t,
    target_for(emsp_1(), "https://emsp-1-NEW.example", "tok-NEW"))
  let count_ok := assert_eq_int(3, hub.peer_count(t2),
                                  "overwrite keeps count")
  let url_ok := match hub.lookup(t2, emsp_1()) {
    Some(target) => assert_eq_str("https://emsp-1-NEW.example",
                                    target.base_url, "overwrite url"),
...
fn test_clientinfo_path_shape() -> Result[Unit, Str] {
  assert_eq_str("/clientinfo/NL/EXM/uid-1",
                hub.clientinfo_path(cpo_a(), "uid-1"),
                "clientinfo path")
}
...
fn test_forward_loop_refused() -> [net, time] Result[Unit, Str] {
  # from == to → LoopDetected before touching the network.
  let r := hub.forward(client.no_retry_policy(),
                       three_peer_table(),
                       emsp_1(), emsp_1(),
                       "GET", "/locations/DE/ABC/L1", None)
  match r {
...
fn test_forward_unknown_receiver() -> [net, time] Result[Unit, Str] {
  # to_party not in the table → UnknownReceiver before touching network.
  let r := hub.forward(client.no_retry_policy(),
                       three_peer_table(),
                       hub_party(), party.new("XX", "YYY"),
                       "GET", "/locations/XX/YYY/L1", None)
  match r {
    Err(UnknownReceiver(p)) => assert_eq_str("XX|YYY", hub.key_of(p),
                                              "unknown receiver key"),
...
fn test_build_method_url() -> Result[Unit, Str] {
  let r := hub.build_forward_request(
    target_emsp_1(), cpo_a(), "PUT", "/locations/NL/EXM/L1",
    Some(JObj([("id", JStr("L1"))])))
  let m_ok := assert_eq_str("PUT", r.method, "method")
  let u_ok := assert_eq_str(
    "https://emsp-1.example/ocpi/2.2.1/locations/NL/EXM/L1",
    r.url, "url")
  and_ok(m_ok, u_ok)
}
...
fn test_build_uses_target_token() -> Result[Unit, Str] {
  let r := hub.build_forward_request(
    target_emsp_1(), cpo_a(), "GET", "/locations", None)
...
fn test_build_preserves_from_party() -> Result[Unit, Str] {
  # The hub forwards the ORIGINAL sender's from-party. The receiver
  # needs to know it's CPO A's request, not the hub's.
  let r := hub.build_forward_request(
    target_emsp_1(), cpo_a(), "GET", "/locations", None)
  ...
  match cc {
    ...
    Some(c) => match pi {
      ...
      Some(p) => and_ok(
        assert_eq_str("NL", c, "from-country"),
        assert_eq_str("EXM", p, "from-party")),
    },
  }
}
...
fn test_build_sets_to_party_from_target() -> Result[Unit, Str] {
  ...
  match cc {
    ...
    Some(c) => match pi {
      ...
      Some(p) => and_ok(
        assert_eq_str("DE", c, "to-country = recipient"),
        assert_eq_str("ABC", p, "to-party = recipient")),
    },
  }
}
...
fn test_error_status_unknown_receiver() -> Result[Unit, Str] {
  assert_eq_int(4002,
    hub.error_to_status_code(UnknownReceiver(emsp_1())),
    "UnknownReceiver -> 4002")
}

fn test_error_status_loop_detected() -> Result[Unit, Str] {
  assert_eq_int(4001,
    hub.error_to_status_code(LoopDetected({ from: cpo_a(), to: cpo_a() })),
    "LoopDetected -> 4001")
}

fn test_error_status_forward_failed() -> Result[Unit, Str] {
  assert_eq_int(4004,
    hub.error_to_status_code(ForwardFailed(HttpFailed("refused"))),
    "ForwardFailed -> 4004")
}

fn test_error_status_matches_status_module() -> Result[Unit, Str] {
  # Sanity check: every status code we emit is actually in
  # `status`'s hub-error catalogue.
  let a := assert_eq_int(4002, status.unknown_receiver(),
                          "status.unknown_receiver")
  let b := assert_eq_int(4001, status.missing_or_invalid_parameters(),
                          "status.missing_or_invalid_parameters")
  let c := assert_eq_int(4004, status.connection_problem(),
...
fn pure_suite() -> List[Result[Unit, Str]] {
  [
    ...
    test_add_then_count(),
    test_add_three_count(),
    test_lookup_hit(),
    test_lookup_miss(),
    test_remove_peer(),
    test_remove_unknown_is_noop(),
    test_add_overwrites(),
    test_all_parties_count(),
    # key_of / clientinfo_path
    test_key_of_shape(),
    test_clientinfo_path_shape(),
    # build_forward_request
    test_build_method_url(),
    test_build_uses_target_token(),
    test_build_preserves_from_party(),
    test_build_sets_to_party_from_target(),
    test_build_get_no_body(),
    test_build_put_with_body(),
    # error_to_status_code mapping
    test_error_status_unknown_receiver(),
    test_error_status_loop_detected(),
    test_error_status_forward_failed(),
    test_error_status_matches_status_module(),
  ]
}

fn fwd_suite() -> [net, time] List[Result[Unit, Str]] {
  [
    test_forward_loop_refused(),
    test_forward_unknown_receiver(),
  ]
}
