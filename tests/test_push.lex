...
fn base() -> Str { "https://emsp.example/ocpi/2.2.1" }
fn b230() -> Str { "https://emsp.example/ocpi/2.3.0" }

fn empty_body() -> jv.Json { JNull }

fn loc_put(loc_id :: Str) -> push.PushKind {
  LocationPut({
    country_code: "NL", party_id: "TNM", location_id: loc_id,
    body: empty_body(),
  })
}
...
fn loc_patch(loc_id :: Str, body :: jv.Json) -> push.PushKind {
  LocationPatch({
    country_code: "NL", party_id: "TNM", location_id: loc_id, body: body,
  })
}
...
fn evse_patch(loc_id :: Str, evse :: Str, body :: jv.Json) -> push.PushKind {
  EvsePatch({
    country_code: "NL", party_id: "TNM", location_id: loc_id,
    evse_uid: evse, body: body,
  })
}

fn conn_patch(
  loc_id :: Str, evse :: Str, conn :: Str, body :: jv.Json
) -> push.PushKind {
  ConnectorPatch({
    country_code: "NL", party_id: "TNM", location_id: loc_id,
    evse_uid: evse, connector_id: conn, body: body,
  })
}

fn sess_put(sid :: Str, body :: jv.Json) -> push.PushKind {
  SessionPut({
    country_code: "NL", party_id: "TNM", session_id: sid, body: body,
  })
}
...
fn tok_put(uid :: Str, body :: jv.Json) -> push.PushKind {
  TokenPut({
    country_code: "DE", party_id: "ABC", token_uid: uid, body: body,
  })
}
...
fn test_method_location_patch() -> Result[Unit, Str] {
  assert_eq_str("PATCH", push.push_method(loc_patch("L1", JNull)),
                "LocationPatch")
}

fn test_method_evse_patch() -> Result[Unit, Str] {
  assert_eq_str("PATCH", push.push_method(evse_patch("L1", "E1", JNull)),
                "EvsePatch")
}

fn test_method_connector_patch() -> Result[Unit, Str] {
  assert_eq_str("PATCH",
    push.push_method(conn_patch("L1", "E1", "C1", JNull)),
    "ConnectorPatch")
}

fn test_method_session_put() -> Result[Unit, Str] {
  assert_eq_str("PUT", push.push_method(sess_put("S1", JNull)),
                "SessionPut")
}

fn test_method_session_patch() -> Result[Unit, Str] {
  assert_eq_str("PATCH", push.push_method(sess_patch("S1", JNull)),
                "SessionPatch")
}
...
fn test_method_token_put() -> Result[Unit, Str] {
  assert_eq_str("PUT", push.push_method(tok_put("RFID-A", JNull)),
                "TokenPut")
}
...
fn test_url_location_put() -> Result[Unit, Str] {
  assert_eq_str(
    ...
    push.push_url(base(), loc_put("L1")),
    "LocationPut URL")
}

fn test_url_location_patch() -> Result[Unit, Str] {
  # PATCH targets the same URL as PUT — method differentiates the op.
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L42",
    push.push_url(base(), loc_patch("L42", JNull)),
    "LocationPatch URL")
}

fn test_url_evse_patch() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L1/EVSE-1",
    push.push_url(base(), evse_patch("L1", "EVSE-1", JNull)),
    "EvsePatch URL")
}

fn test_url_connector_patch() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L1/EVSE-1/1",
    push.push_url(base(), conn_patch("L1", "EVSE-1", "1", JNull)),
    "ConnectorPatch URL")
}

fn test_url_session_put() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/sessions/NL/TNM/S-7",
    push.push_url(base(), sess_put("S-7", JNull)),
    "SessionPut URL")
}

fn test_url_session_patch() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.3.0/sessions/NL/TNM/S-7",
    push.push_url(b230(), sess_patch("S-7", JNull)),
    "SessionPatch URL")
}

fn test_url_cdr_post() -> Result[Unit, Str] {
  # CDRs don't have a per-party path segment — the receiver picks
  # its own id and routes by the OCPI-from-* headers.
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/cdrs",
    push.push_url(base(), cdr_post(JNull)),
    "CdrPost URL")
}

fn test_url_token_put() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/tokens/DE/ABC/RFID-A",
    push.push_url(base(), tok_put("RFID-A", JNull)),
    "TokenPut URL")
}
...
fn test_body_transparent_location_put() -> Result[Unit, Str] {
  ...
  let got  := push.push_body(LocationPut({
    country_code: "NL", party_id: "TNM", location_id: "L1", body: body,
  }))
...
fn test_location_url_helper() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L1",
    push.location_url(base(), "NL", "TNM", "L1"),
    "location_url")
}

fn test_evse_url_helper() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L1/EVSE-9",
    push.evse_url(base(), "NL", "TNM", "L1", "EVSE-9"),
    "evse_url")
}

fn test_connector_url_helper() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/locations/NL/TNM/L1/EVSE-9/2",
    push.connector_url(base(), "NL", "TNM", "L1", "EVSE-9", "2"),
    "connector_url")
}
...
fn test_session_url_helper() -> Result[Unit, Str] {
  assert_eq_str(
    ...
    push.session_url(b230(), "DE", "ABC", "S-7"),
    "session_url")
}

fn test_token_url_helper() -> Result[Unit, Str] {
  assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/tokens/DE/ABC/RFID-Z",
    push.token_url(base(), "DE", "ABC", "RFID-Z"),
    "token_url")
}
...
fn cpo_party() -> party.PartyId { party.new("NL", "EXM") }
fn emsp_party() -> party.PartyId { party.new("DE", "ABC") }

fn target() -> push.PushTarget {
  { party: emsp_party(),
    base_url: base(),
    token: "secret-token-b64" }
}
...
fn test_build_method_url_cdr() -> Result[Unit, Str] {
  ...
  let u_ok := assert_eq_str(
    "https://emsp.example/ocpi/2.2.1/cdrs", r.url, "CDR url")
  and_ok(m_ok, u_ok)
}
...
fn test_build_has_json_body() -> Result[Unit, Str] {
  ...
  let body_ok := match r.body {
    ...
    Some(_) => pass(),     # bytes; not asserting exact contents here
  }
...
fn suite() -> List[Result[Unit, Str]] {
  [
    # push_method
    test_method_location_put(),
    test_method_location_patch(),
    test_method_evse_patch(),
    test_method_connector_patch(),
    test_method_session_put(),
    test_method_session_patch(),
    test_method_cdr_post(),
    test_method_token_put(),
    # push_url
    test_url_location_put(),
    test_url_location_patch(),
    test_url_evse_patch(),
    test_url_connector_patch(),
    test_url_session_put(),
    test_url_session_patch(),
    test_url_cdr_post(),
    test_url_token_put(),
    # push_body
    test_body_transparent_location_put(),
    test_body_transparent_cdr_post(),
    # URL helpers
    test_location_url_helper(),
    test_evse_url_helper(),
    test_connector_url_helper(),
    test_session_url_helper(),
    test_token_url_helper(),
    # build_request
    test_build_method_url_cdr(),
    test_build_method_url_evse(),
    test_build_has_authorization(),
    test_build_has_from_party(),
    test_build_has_to_party(),
    test_build_has_json_body(),
  ]
}
