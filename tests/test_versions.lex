...
fn test_version_constants() -> Result[Unit, Str] {
  if versions.v211() == "2.1.1" and versions.v221() == "2.2.1" and versions.v230() == "2.3.0" {
    pass()
  } else {
...
fn test_endpoint_sender() -> Result[Unit, Str] {
  let e := versions.endpoint_sender(mid.locations(), "https://example.com/ocpi/cpo/2.2.1/locations")
  if e.role == iface.sender() {
    pass()
  } else {
    fail("endpoint_sender role should be SENDER")
  }
}

fn test_endpoint_receiver() -> Result[Unit, Str] {
  let e := versions.endpoint_receiver(mid.credentials(), "https://example.com/ocpi/cpo/2.2.1/credentials")
  if e.role == iface.receiver() {
    pass()
  } else {
    fail("endpoint_receiver role should be RECEIVER")
  }
}
...
fn test_endpoint_to_json_shape() -> Result[Unit, Str] {
  ...
  assert_true(str.contains(raw, "\"identifier\":\"locations\"") and str.contains(raw, "\"role\":\"SENDER\"") and str.contains(raw, "\"url\":\"https://example.com/ocpi/cpo/2.2.1/locations\""), "endpoint JSON shape")
}

# ---- VersionDetail building -------------------------------------
fn test_detail_to_json() -> Result[Unit, Str] {
  let d := versions.detail(versions.v221(), versions.standard_cpo_v221_endpoints("https://example.com/ocpi/cpo/2.2.1"))
  let raw := jv.stringify(versions.detail_to_json(d))
  assert_true(str.contains(raw, "\"version\":\"2.2.1\"") and str.contains(raw, "\"identifier\":\"locations\"") and str.contains(raw, "\"identifier\":\"credentials\""), "detail JSON shape")
}
...
fn test_standard_emsp_endpoints_count() -> Result[Unit, Str] {
  ...
  if list.len(eps) == 7 {
    ...
    fail("standard eMSP v2.2.1 should list 7 endpoints")
  }
}
