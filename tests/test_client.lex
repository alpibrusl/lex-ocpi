...
fn test_with_token() -> Result[Unit, Str] {
  let r := client.with_token(client.base_request("GET", "https://example.com/x"), "ABC==")
...
fn test_with_party_routing() -> Result[Unit, Str] {
  let r := client.with_party_routing(client.base_request("GET", "https://example.com/x"), party.new("NL", "TNM"), party.new("DE", "BMW"))
  ...
  if from_cc == "NL" and to_pid == "BMW" {
    pass()
  } else {
    fail("party routing headers missing or wrong")
  }
}

# ---- with_json_body sets Content-Type and body -----------------
fn test_with_json_body_sets_ct() -> Result[Unit, Str] {
  let r := client.with_json_body(client.base_request("PUT", "https://example.com/x"), "{\"hello\":\"world\"}")
...
fn test_with_request_id() -> Result[Unit, Str] {
  let r := client.with_request_id(client.base_request("GET", "https://example.com/x"), "req-1")
...
fn suite() -> List[Result[Unit, Str]] {
  [test_base_request_method(), test_base_request_url(), test_with_token(), test_with_party_routing(), test_with_json_body_sets_ct(), test_with_request_id()]
}
