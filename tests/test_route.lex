...
fn empty_headers() -> headers.OcpiHeaders {
  headers.new("", "", "", party.new("", ""), party.new("", ""))
}

fn empty_req(method :: Str, module :: Str) -> route.OcpiRequest {
  route.request(method, module, str.concat("/", module), map.new(), map.new(), empty_headers(), JNull)
}

fn req_with_body(method :: Str, module :: Str, body :: jv.Json) -> route.OcpiRequest {
  route.request(method, module, str.concat("/", module), map.new(), map.new(), empty_headers(), body)
}
...
fn test_dispatch_method_mismatch() -> Result[Unit, Str] {
  ...
  if res.status_code != 1000 {
    ...
    fail("POST should not match a GET-only route")
  }
}
...
fn test_validator_short_circuits() -> Result[Unit, Str] {
  let reg := route.handler_with_schema(route.new(), route.post(), "locations", always_fail_validator, hello_handler)
  let res := route.dispatch(reg, req_with_body(route.post(), "locations", JObj([])), ts())
  assert_eq_int(2001, res.status_code, "validator failure should return 2001")
}

fn test_validator_passes_through() -> Result[Unit, Str] {
  let reg := route.handler_with_schema(route.new(), route.post(), "locations", always_pass_validator, hello_handler)
  let res := route.dispatch(reg, req_with_body(route.post(), "locations", JObj([])), ts())
...
fn suite() -> List[Result[Unit, Str]] {
  [test_dispatch_known_route(), test_dispatch_unknown_route(), test_dispatch_method_mismatch(), test_handler_returns_error(), test_validator_short_circuits(), test_validator_passes_through()]
}
