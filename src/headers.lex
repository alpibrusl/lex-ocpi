...
type OcpiHeaders = { authorization :: Str, request_id :: Str, correlation_id :: Str, from_party :: party.PartyId, to_party :: party.PartyId }

fn new(authorization :: Str, request_id :: Str, correlation_id :: Str, from_party :: party.PartyId, to_party :: party.PartyId) -> OcpiHeaders {
  { authorization: authorization, request_id: request_id, correlation_id: correlation_id, from_party: from_party, to_party: to_party }
}
...
fn h_from_country_code() -> Str {
  "ocpi-from-country-code"
}

fn h_from_party_id() -> Str {
  "ocpi-from-party-id"
}

fn h_to_country_code() -> Str {
  "ocpi-to-country-code"
}

fn h_to_party_id() -> Str {
  "ocpi-to-party-id"
}
...
fn from_map(headers :: Map[Str, Str]) -> OcpiHeaders {
  new(get_or_empty(headers, h_authorization()), get_or_empty(headers, h_request_id()), get_or_empty(headers, h_correlation_id()), party.new(get_or_empty(headers, h_from_country_code()), get_or_empty(headers, h_from_party_id())), party.new(get_or_empty(headers, h_to_country_code()), get_or_empty(headers, h_to_party_id())))
}
  ...
  }
{
  ...
  if str.len(authz) <= str.len(prefix) {
    ...
    if str.slice(authz, 0, str.len(prefix)) == prefix {
...
fn has_party_routing(h :: OcpiHeaders) -> Bool {
  not str.is_empty(h.from_party.country_code) and not str.is_empty(h.from_party.party_id) and not str.is_empty(h.to_party.country_code) and not str.is_empty(h.to_party.party_id)
}
