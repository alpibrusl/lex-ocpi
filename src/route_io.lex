...
# ---- Registry datatype -------------------------------------------
type IoRouteEntry = { method :: Str, module :: Str, validator :: Option[(jv.Json) -> Result[jv.Json, List[e.Error]]], handler :: (route.OcpiRequest) -> [io, time, sql] route.HandlerResult }

type IoRegistry = { routes :: List[IoRouteEntry], on_unknown :: (route.OcpiRequest) -> [io, time, sql] route.HandlerResult }
...
fn default_unknown(req :: route.OcpiRequest) -> [io, time, sql] route.HandlerResult {
  route.fail(oe.err(status.client_error(), str.concat("no handler for ", str.concat(req.method, str.concat(" ", req.module)))))
}
...
fn handler(reg :: IoRegistry, method :: Str, module :: Str, h :: IoHandler) -> IoRegistry {
  add_entry(reg, { method: method, module: module, validator: None, handler: h })
}

fn handler_with_schema(reg :: IoRegistry, method :: Str, module :: Str, validator :: route.Validator, h :: IoHandler) -> IoRegistry {
  add_entry(reg, { method: method, module: module, validator: Some(validator), handler: h })
}
...
# ---- Dispatch ----------------------------------------------------
fn dispatch(reg :: IoRegistry, req :: route.OcpiRequest, timestamp :: Str) -> [io, time, sql] env.OcpiResponse {
...
fn run_entry(entry :: IoRouteEntry, req :: route.OcpiRequest, timestamp :: Str) -> [io, time, sql] env.OcpiResponse {
  match entry.validator {
    None => response_from_handler(entry.handler(req), timestamp),
    Some(vf) => match vf(req.body) {
      Err(es) => response_from_handler(route.fail(oe.from_schema_errors(es)), timestamp),
      Ok(normalized) => {
        let r2 := route.request(req.method, req.module, req.path, req.path_params, req.query, req.headers, normalized)
