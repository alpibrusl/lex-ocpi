...
fn cprt_accepted() -> Str {
  "ACCEPTED"
}

fn cprt_not_supported() -> Str {
  "NOT_SUPPORTED"
}

fn cprt_rejected() -> Str {
  "REJECTED"
}

fn cprt_too_often() -> Str {
  "TOO_OFTEN"
}

fn cprt_unknown_session() -> Str {
  "UNKNOWN_SESSION"
}

fn all_charging_profile_response_type() -> List[Str] {
  [cprt_accepted(), cprt_not_supported(), cprt_rejected(), cprt_too_often(), cprt_unknown_session()]
}

# ---- ChargingProfilePeriod --------------------------------------
fn charging_profile_period_schema() -> s.ModelSchema {
  { title: "ChargingProfilePeriod", description: "OCPI 2.2.1 — one slice of a ChargingProfile", fields: [s.required_int("start_period", [IntNonNegative]), s.required_float("limit", [])] }
}

# ---- ChargingProfile --------------------------------------------
fn charging_profile_schema() -> s.ModelSchema {
  { title: "ChargingProfile", description: "OCPI 2.2.1 — ChargingProfile object", fields: [s.optional(s.required_str("start_date_time", [StrNonEmpty])), s.optional(s.required_int("duration", [IntPositive])), s.required_str("charging_rate_unit", [StrOneOf(all_charging_rate_unit())]), s.optional(s.required_float("min_charging_rate", [])), s.required_array("charging_profile_period", KObject(charging_profile_period_schema()), [ListNonEmpty])] }
}
...
fn active_charging_profile_schema() -> s.ModelSchema {
  { title: "ActiveChargingProfile", description: "OCPI 2.2.1 — ActiveChargingProfile object", fields: [s.required_str("start_date_time", [StrNonEmpty]), s.required_object("charging_profile", charging_profile_schema())] }
}
...
fn set_charging_profile_schema() -> s.ModelSchema {
  { title: "SetChargingProfile", description: "OCPI 2.2.1 — SetChargingProfile PUT body", fields: [s.required_object("charging_profile", charging_profile_schema()), s.required_str("response_url", [StrNonEmpty, StrMaxLen(255)])] }
}
...
fn charging_profile_response_schema() -> s.ModelSchema {
  { title: "ChargingProfileResponse", description: "OCPI 2.2.1 — synchronous ChargingProfileResponse", fields: [s.required_str("result", [StrOneOf(all_charging_profile_response_type())]), s.required_int("timeout", [IntNonNegative])] }
}
...
fn active_charging_profile_result_schema() -> s.ModelSchema {
  { title: "ActiveChargingProfileResult", description: "OCPI 2.2.1 — async ActiveChargingProfileResult", fields: [s.required_str("result", [StrOneOf(all_charging_profile_result_type())]), s.optional(s.required_object("profile", active_charging_profile_schema()))] }
}
...
fn charging_profile_result_schema() -> s.ModelSchema {
  { title: "ChargingProfileResult", description: "OCPI 2.2.1 — async ChargingProfileResult", fields: [s.required_str("result", [StrOneOf(all_charging_profile_result_type())])] }
}
