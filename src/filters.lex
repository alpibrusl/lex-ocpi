...
type DateRange = { date_from :: Option[Str], date_to :: Option[Str] }

fn from_query(query :: Map[Str, Str]) -> DateRange {
  { date_from: map.get(query, "date_from"), date_to: map.get(query, "date_to") }
}
...
fn apply(items :: List[jv.Json], range :: DateRange) -> List[jv.Json] {
  list.filter(items, fn (item :: jv.Json) -> Bool {
    in_range(item, range)
  })
}
...
fn in_range(item :: jv.Json, range :: DateRange) -> Bool {
  match jv.get_field(item, "last_updated") {
    ...
    Some(v) => match jv.as_str(v) {
      None => false,
      Some(ts) => above_lower(ts, range.date_from) and below_upper(ts, range.date_to),
    },
  }
}
