...
fn invalid_or_missing_parameters() -> Int {
  2001
}

fn not_enough_information() -> Int {
  2002
}

fn unknown_location() -> Int {
  2003
}

fn unknown_token() -> Int {
  2004
}
...
fn unable_to_use_api() -> Int {
  3001
}

fn unsupported_version() -> Int {
  3002
}

fn no_matching_endpoints() -> Int {
  3003
}
...
fn missing_or_invalid_parameters() -> Int {
  4001
}

fn unknown_receiver() -> Int {
  4002
}

fn timeout_on_forwarded_request() -> Int {
  4003
}

fn connection_problem() -> Int {
  4004
}
...
fn all_codes() -> List[Int] {
  [success(), client_error(), invalid_or_missing_parameters(), not_enough_information(), unknown_location(), unknown_token(), server_error(), unable_to_use_api(), unsupported_version(), no_matching_endpoints(), hub_error(), missing_or_invalid_parameters(), unknown_receiver(), timeout_on_forwarded_request(), connection_problem()]
}
  ...
  }
{
  if code == success() {
    ...
    if code == client_error() {
      ...
      if code == invalid_or_missing_parameters() {
        "Invalid or missing parameters"
      } else {
        if code == not_enough_information() {
          ...
          if code == unknown_location() {
            ...
            if code == unknown_token() {
              ...
              if code == server_error() {
                ...
                if code == unable_to_use_api() {
                  "Unable to use the client's API"
                } else {
                  if code == unsupported_version() {
                    "Unsupported version"
                  } else {
                    if code == no_matching_endpoints() {
                      "No matching endpoints or expected endpoints missing"
                    } else {
                      if code == hub_error() {
                        "Generic Hub error"
                      } else {
                        if code == missing_or_invalid_parameters() {
                          "Missing or invalid parameters"
                        } else {
                          if code == unknown_receiver() {
                            "Unknown receiver"
                          } else {
                            if code == timeout_on_forwarded_request() {
                              "Timeout on forwarded request"
                            } else {
                              if code == connection_problem() {
                                "Connection problem"
                              } else {
                                ""
                              }
