...
type CommandType = StartSession | StopSession | ReserveNow | CancelReservation | UnlockConnector

fn cmd_start_session_str() -> Str {
  "START_SESSION"
}

fn cmd_stop_session_str() -> Str {
  "STOP_SESSION"
}

fn cmd_reserve_now_str() -> Str {
  "RESERVE_NOW"
}

fn cmd_cancel_reservation_str() -> Str {
  "CANCEL_RESERVATION"
}

fn cmd_unlock_connector_str() -> Str {
  "UNLOCK_CONNECTOR"
}
...
fn decode_command_type(s :: Str) -> Result[CommandType, Str] {
  if s == cmd_start_session_str() {
    Ok(StartSession)
  } else {
    if s == cmd_stop_session_str() {
      Ok(StopSession)
    } else {
      if s == cmd_reserve_now_str() {
        Ok(ReserveNow)
      } else {
        if s == cmd_cancel_reservation_str() {
          Ok(CancelReservation)
        } else {
          if s == cmd_unlock_connector_str() {
            Ok(UnlockConnector)
          } else {
            Err(str.concat("CommandType not in catalogue: ", s))
          }
        }
      }
    }
...
type CommandResponseType = CrAccepted | CrRejected | CrNotSupported | CrUnknownSession

fn cmdr_accepted_str() -> Str {
  "ACCEPTED"
}

fn cmdr_rejected_str() -> Str {
  "REJECTED"
}

fn cmdr_not_supported_str() -> Str {
  "NOT_SUPPORTED"
}

fn cmdr_unknown_session_str() -> Str {
  "UNKNOWN_SESSION"
}
...
fn decode_command_response_type(s :: Str) -> Result[CommandResponseType, Str] {
  if s == cmdr_accepted_str() {
    Ok(CrAccepted)
  } else {
    if s == cmdr_rejected_str() {
      Ok(CrRejected)
    } else {
      if s == cmdr_not_supported_str() {
        Ok(CrNotSupported)
      } else {
        if s == cmdr_unknown_session_str() {
          Ok(CrUnknownSession)
        } else {
          Err(str.concat("CommandResponseType not in catalogue: ", s))
        }
      }
    }
  }
...
type CommandResultType = ResAccepted | ResCanceledReservation | ResEvseOccupied | ResEvseInoperative | ResFailed | ResNotSupported | ResRejected | ResTimeout | ResUnknownReservation

fn res_accepted_str() -> Str {
  "ACCEPTED"
}

fn res_canceled_reservation_str() -> Str {
  "CANCELED_RESERVATION"
}

fn res_evse_occupied_str() -> Str {
  "EVSE_OCCUPIED"
}

fn res_evse_inoperative_str() -> Str {
  "EVSE_INOPERATIVE"
}

fn res_failed_str() -> Str {
  "FAILED"
}

fn res_not_supported_str() -> Str {
  "NOT_SUPPORTED"
}

fn res_rejected_str() -> Str {
  "REJECTED"
}

fn res_timeout_str() -> Str {
  "TIMEOUT"
}

fn res_unknown_reservation_str() -> Str {
  "UNKNOWN_RESERVATION"
}
...
fn decode_command_result_type(s :: Str) -> Result[CommandResultType, Str] {
  if s == res_accepted_str() {
    Ok(ResAccepted)
  } else {
    if s == res_canceled_reservation_str() {
      Ok(ResCanceledReservation)
    } else {
      if s == res_evse_occupied_str() {
        Ok(ResEvseOccupied)
      } else {
        if s == res_evse_inoperative_str() {
          Ok(ResEvseInoperative)
        } else {
          if s == res_failed_str() {
            Ok(ResFailed)
          } else {
            if s == res_not_supported_str() {
              Ok(ResNotSupported)
            } else {
              if s == res_rejected_str() {
                Ok(ResRejected)
              } else {
                if s == res_timeout_str() {
                  Ok(ResTimeout)
                } else {
                  if s == res_unknown_reservation_str() {
                    Ok(ResUnknownReservation)
                  } else {
                    Err(str.concat("CommandResultType not in catalogue: ", s))
                  }
                }
              }
            }
...
type CommandResponse = { result :: CommandResponseType, timeout :: Option[Int], messages :: List[jv.Json] }

fn response(res :: CommandResponseType, timeout :: Option[Int], messages :: List[jv.Json]) -> CommandResponse {
...
fn accepted(timeout :: Option[Int]) -> CommandResponse
  examples {
    accepted(Some(30)) => { result: CrAccepted, timeout: Some(30), messages: [] }
  }
{
  ...
  match jv.get_field(j, "result") {
    ...
    Some(rv) => match jv.as_str(rv) {
      ...
      Some(s) => match decode_command_response_type(s) {
        Err(why) => Err(why),
        Ok(t) => Ok(response(t, decode_optional_timeout(j), decode_messages(j))),
      },
    },
  }
}
...
type CommandResult = { result :: CommandResultType, messages :: List[jv.Json] }

fn make_result(rt :: CommandResultType, messages :: List[jv.Json]) -> CommandResult {
  { result: rt, messages: messages }
}

fn result_accepted() -> CommandResult {
  make_result(ResAccepted, [])
}

fn result_failed() -> CommandResult {
  make_result(ResFailed, [])
}

fn result_rejected() -> CommandResult {
  make_result(ResRejected, [])
}

fn result_timeout() -> CommandResult {
  make_result(ResTimeout, [])
}

fn result_evse_occupied() -> CommandResult {
  make_result(ResEvseOccupied, [])
}

fn result_evse_inoperative() -> CommandResult {
  make_result(ResEvseInoperative, [])
}
...
# own route.
fn command_handler(handle :: (jv.Json, Str) -> CommandResponse) -> (route.OcpiRequest) -> route.HandlerResult {
  fn (req :: route.OcpiRequest) -> route.HandlerResult {
    match response_url(req.body) {
      None => route.fail(oe.invalid_parameters("command body missing `response_url`")),
      Some(url) => {
...
fn submit_command(commands_base :: Str, cmd_type :: CommandType, body :: jv.Json, token_b64 :: Str) -> [net] Result[CommandResponse, client.ClientError] {
...
fn build_command_url(base :: Str, cmd_type :: CommandType) -> Str
  examples {
    build_command_url("https://cpo.example/ocpi/2.2.1/commands", StartSession) => "https://cpo.example/ocpi/2.2.1/commands/START_SESSION"
  }
{
