...
# `pagination.Page`.
#
# Effects: [sql] for `list_paginated`.
...
# Bounds are `[date_from, date_to)` per OCPI 2.2.1 §4.3 — inclusive
# lower, exclusive upper. Either bound may be None.
fn apply_date_range(sel :: q.SelectQuery, range :: ft.DateRange) -> q.SelectQuery {
...
# carrying the slice for this page and the total row count
# (consumed by `X-Total-Count`).
#
# `decode` is threaded through lex-orm 0.9-style; for OCPI use the
# per-module passthrough decoder (`r_locs.decode`, `r_sess.decode`,
# etc.).
fn list_paginated(repo :: q.RepoSchema, decode :: (jv.Json) -> Result[jv.Json, se.Errors], db :: conn.ConnDb, req :: pg.PageRequest, range :: ft.DateRange) -> [sql] Result[pg.Page, dbe.DbErr] {
  ...
  match q.run_select(paged, decode, db) {
    Err(e) => Err(e),
