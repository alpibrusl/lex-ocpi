...
# call is idempotent.
#
# This bypasses lex-orm's `m.apply` version-based runner because
# that runner evolves a single ModelSchema across versions — it
# doesn't fit a multi-table schema (a v2=Sessions would diff against
# v1=Locations and emit nonsense ALTERs). When lex-orm grows
# multi-table migrations we can switch over.
