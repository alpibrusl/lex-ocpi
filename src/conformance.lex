...
type ConformanceError =
    EnvelopeFieldEmpty(Str)                 # which field
  | StatusCodeOutOfBand(Int)                 # not in 1xxx-4xxx
  | StatusMessageMissing(Int)                # non-1xxx code with empty message
  | StatusMessageOnSuccess                   # 1xxx code carries a non-empty message
                                              #   (allowed by spec but unusual; advisory)
  | HeaderMissing(Str)                       # which header name
  | HeaderMalformed({ name :: Str, why :: Str })
  | HeaderEchoMismatch({ name :: Str,
                         sent :: Str, received :: Str })
  | PaginationFieldMissing(Str)              # which pagination header
  | PaginationFieldInvalid({ name :: Str, value :: Str, why :: Str })
  | DataShape(Str)                            # data field doesn't match the
                                              #   expected shape for an ok response

# Render a single ConformanceError as Str.
fn render(e :: ConformanceError) -> Str {
  match e {
    EnvelopeFieldEmpty(f)          => str.concat("envelope field empty: ", f),
    StatusCodeOutOfBand(c)         => str.concat("status_code out of band (1xxx-4xxx): ",
                                                  int.to_str(c)),
    StatusMessageMissing(c)        => str.concat("status_message empty for non-success code ",
                                                  int.to_str(c)),
    StatusMessageOnSuccess         => "status_message non-empty for 1xxx success",
    HeaderMissing(name)            => str.concat("header missing: ", name),
    HeaderMalformed(d)             => str.concat(str.concat("header ", d.name),
                                                  str.concat(" malformed: ", d.why)),
    HeaderEchoMismatch(d)          => str.concat(str.concat("header ", d.name),
                                                  str.concat(" not echoed: sent=",
                                                    str.concat(d.sent,
                                                      str.concat(" received=", d.received)))),
    PaginationFieldMissing(n)      => str.concat("pagination header missing: ", n),
    PaginationFieldInvalid(d)      => str.concat(str.concat("pagination ", d.name),
                                                  str.concat(" invalid (", str.concat(d.value,
                                                    str.concat("): ", d.why)))),
...
{
  if code >= 1000 and code < 2000 { Success }
  else { if code >= 2000 and code < 3000 { ClientError }
  else { if code >= 3000 and code < 4000 { ServerError }
  else { if code >= 4000 and code < 5000 { HubError }
  else { Unknown } } } }
}
...
fn check_envelope(r :: env.OcpiResponse) -> Result[Unit, ConformanceError]
  examples {
    check_envelope({ data: JNull, status_code: 1000,
                     status_message: "", timestamp: "2026-05-16T00:00:00Z" }) =>
      Ok(()),
    check_envelope({ data: JNull, status_code: 1000,
                     status_message: "", timestamp: "" }) =>
      Err(EnvelopeFieldEmpty("timestamp")),
    check_envelope({ data: JNull, status_code: 9999,
                     status_message: "", timestamp: "2026-05-16T00:00:00Z" }) =>
      Err(StatusCodeOutOfBand(9999)),
    check_envelope({ data: JNull, status_code: 2003,
                     status_message: "", timestamp: "2026-05-16T00:00:00Z" }) =>
      Err(StatusMessageMissing(2003)),
  }
{
...
fn check_message_for_code(code :: Int, message :: Str) -> Result[Unit, ConformanceError] {
  match classify(code) {
    Success => Ok(()),                       # spec allows empty message on success
    _       => if message == "" {
...
fn check_module_request_headers(req :: h.OcpiHeaders) -> Result[Unit, ConformanceError]
  examples {
    check_module_request_headers(h.new("Token x", "rid-1", "corr-1",
      party.new("NL", "EXM"), party.new("DE", "ABC"))) => Ok(()),
    check_module_request_headers(h.new("", "rid", "corr",
      party.new("NL", "EXM"), party.new("DE", "ABC"))) =>
      Err(HeaderMissing("authorization")),
    check_module_request_headers(h.new("Bearer x", "rid", "corr",
      party.new("NL", "EXM"), party.new("DE", "ABC"))) =>
      Err(HeaderMalformed({ name: "authorization", why: "must start with 'Token '" })),
  }
{
...
fn check_authorization(auth :: Str) -> Result[Unit, ConformanceError] {
  ...
  } else { if str.starts_with(auth, "Token ") {
    Ok(())
  } else {
    Err(HeaderMalformed({ name: h.h_authorization(),
                          why: "must start with 'Token '" }))
  } }
}

fn check_non_empty(s :: Str, name :: Str) -> Result[Unit, ConformanceError] {
  if s == "" { Err(HeaderMissing(name)) } else { Ok(()) }
}
...
fn check_party(p :: party.PartyId, side :: Str) -> Result[Unit, ConformanceError] {
  let cc_name := if side == "from" { h.h_from_country_code() }
                 else { h.h_to_country_code() }
  let pi_name := if side == "from" { h.h_from_party_id() }
                 else { h.h_to_party_id() }
  if p.country_code == "" {
    Err(HeaderMissing(cc_name))
  } else { if str.len(p.country_code) != 2 {
    Err(HeaderMalformed({ name: cc_name,
                          why: "country_code must be 2 chars (ISO-3166)" }))
  } else { if p.party_id == "" {
    Err(HeaderMissing(pi_name))
  } else { if str.len(p.party_id) != 3 {
...
fn check_response_echoes_request(
  req :: h.OcpiHeaders,
  resp_headers :: Map[Str, Str]
) -> Result[Unit, ConformanceError] {
...
fn check_echo(
  ...
  match map.get(resp_headers, name) {
    ...
    Some(got)   => if got == sent { Ok(()) }
                   else { Err(HeaderEchoMismatch({
                     name: name, sent: sent, received: got })) },
  }
}
...
fn check_int_header(
  ...
  match map.get(resp_headers, name) {
    ...
    Some(s) => match str.to_int(s) {
      None    => Err(PaginationFieldInvalid({
                   name: name, value: s, why: "not an integer" })),
      Some(n) => if n < 0 {
                   Err(PaginationFieldInvalid({
                     name: name, value: s, why: "must be non-negative" }))
                 } else {
                   Ok(())
                 },
    },
...
fn check_link_header_present(
  ...
  match map.get(resp_headers, "link") {
    ...
    Some(s) => if s == "" {
                 Err(PaginationFieldInvalid({
                   name: "link", value: "", why: "empty" }))
               } else { if str.contains(s, "rel=\"next\"") {
                 Ok(())
               } else {
                 Err(PaginationFieldInvalid({
                   name: "link", value: s, why: "missing rel=\"next\"" }))
               } },
  }
}
...
fn assert_response_echoes_request(
  req :: h.OcpiHeaders,
  resp_headers :: Map[Str, Str]
) -> Result[Unit, Str] {
