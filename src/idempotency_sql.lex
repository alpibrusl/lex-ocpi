...
# ---- Config ----------------------------------------------------
type SqlCacheConfig = { ttl_ms :: Int, poll_interval_ms :: Int, max_wait_ms :: Int }
...
fn setup(db :: Db) -> [sql] Result[Unit, Str] {
  let t := table_name()
  let create_sql := str.concat("CREATE TABLE IF NOT EXISTS ", str.concat(t, " (cache_key TEXT PRIMARY KEY, status TEXT NOT NULL, response_json TEXT, expires_at_ms BIGINT NOT NULL, created_at_ms BIGINT NOT NULL)"))
  match sql.exec(db, create_sql, []) {
    Err(e) => Err(e.message),
    Ok(_) => {
      let idx_sql := str.concat("CREATE INDEX IF NOT EXISTS idx_ocpi_idempotency_expires ON ", str.concat(t, " (expires_at_ms)"))
...
# ---- Reserve outcome --------------------------------------------
type SqlReserve = SqlReserveRun | SqlReserveHit(env.OcpiResponse) | SqlReserveWait

fn try_reserve(db :: Db, key :: Str, now_ms :: Int, expires_at_ms :: Int) -> [sql] SqlReserve {
  let t := table_name()
  let ins_sql := str.concat("INSERT INTO ", str.concat(t, " (cache_key, status, response_json, expires_at_ms, created_at_ms) VALUES ($1, 'inflight', NULL, $2, $3) ON CONFLICT (cache_key) DO NOTHING"))
...
fn inspect_existing(db :: Db, key :: Str, now_ms :: Int) -> [sql] SqlReserve {
  let t := table_name()
  let sel_sql := str.concat("SELECT status, response_json, expires_at_ms FROM ", str.concat(t, " WHERE cache_key = $1"))
  let raw :: Result[List[{ status :: Str, response_json :: Option[Str], expires_at_ms :: Int }], SqlError] := sql.query(db, sel_sql, [PStr(key)])
  match raw {
    Err(_) => SqlReserveWait,
...
fn decode_existing(r :: { status :: Str, response_json :: Option[Str], expires_at_ms :: Int }, now_ms :: Int) -> SqlReserve {
...
# ---- Completion store ------------------------------------------
fn store_completed(db :: Db, key :: Str, resp :: env.OcpiResponse) -> [sql] Result[Unit, Str] {
  let t := table_name()
  let upd_sql := str.concat("UPDATE ", str.concat(t, " SET status = 'completed', response_json = $1 WHERE cache_key = $2"))
...
fn stringify_response(r :: env.OcpiResponse) -> Str {
  jv.stringify(JObj([("data", r.data), ("status_code", JInt(r.status_code)), ("status_message", JStr(r.status_message)), ("timestamp", JStr(r.timestamp))]))
}

fn forget(db :: Db, key :: Str) -> [sql] Result[Unit, Str] {
  let t := table_name()
  let del_sql := str.concat("DELETE FROM ", str.concat(t, " WHERE cache_key = $1"))
...
fn purge_expired(db :: Db, now_ms :: Int) -> [sql] Result[Int, Str] {
  let t := table_name()
  let del_sql := str.concat("DELETE FROM ", str.concat(t, " WHERE expires_at_ms < $1"))
...
fn dispatch_with_cache_sql(reg :: rio.IoRegistry, db :: Db, cfg :: SqlCacheConfig, req :: route.OcpiRequest, timestamp :: Str) -> [io, time, sql] env.OcpiResponse {
...
fn dispatch_branch_sql(reg :: rio.IoRegistry, db :: Db, cfg :: SqlCacheConfig, req :: route.OcpiRequest, timestamp :: Str, key :: Str) -> [io, time, sql] env.OcpiResponse {
  let now := time.now_ms()
...
fn run_and_store(reg :: rio.IoRegistry, db :: Db, req :: route.OcpiRequest, timestamp :: Str, key :: Str) -> [io, time, sql] env.OcpiResponse {
  let resp := rio.dispatch(reg, req, timestamp)
  let __lex_discard_1 := store_completed(db, key, resp)
  resp
}

# Poll until the inflight row completes or `max_wait_ms` elapses.
# Same shape as the in-memory variant's `poll_for_completion`.
fn wait_or_fallback(reg :: rio.IoRegistry, db :: Db, cfg :: SqlCacheConfig, req :: route.OcpiRequest, timestamp :: Str, key :: Str) -> [io, time, sql] env.OcpiResponse {
  let deadline_ns := time.mono_ns() + cfg.max_wait_ms * 1000000
  poll_loop_sql(reg, db, cfg, req, timestamp, key, deadline_ns)
}

fn poll_loop_sql(reg :: rio.IoRegistry, db :: Db, cfg :: SqlCacheConfig, req :: route.OcpiRequest, timestamp :: Str, key :: Str, deadline_ns :: Int) -> [io, time, sql] env.OcpiResponse {
  let now := time.now_ms()
  match inspect_existing(db, key, now) {
    SqlReserveHit(r) => r,
    SqlReserveRun => run_and_store(reg, db, req, timestamp, key),
    SqlReserveWait => if time.mono_ns() >= deadline_ns {
      let __lex_discard_2 := forget(db, key)
      run_and_store(reg, db, req, timestamp, key)
    } else {
      let __lex_discard_3 := time.sleep_ms(cfg.poll_interval_ms)
      poll_loop_sql(reg, db, cfg, req, timestamp, key, deadline_ns)
    },
  }
}
