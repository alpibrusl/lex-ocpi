...
type Image = { url :: Str, thumbnail :: Option[Str], category :: Str, image_type :: Str, width :: Option[Int], height :: Option[Int] }

type BusinessDetails = { name :: Str, website :: Option[Str], logo :: Option[Image] }

fn business_details(name :: Str) -> BusinessDetails {
  { name: name, website: None, logo: None }
}

fn business_details_with(name :: Str, website :: Option[Str], logo :: Option[Image]) -> BusinessDetails {
  { name: name, website: website, logo: logo }
}

fn image_to_json(img :: Image) -> jv.Json {
  let base := [("url", JStr(img.url)), ("category", JStr(img.category)), ("type", JStr(img.image_type))]
...
# business details.
type CredentialsRole = { role :: Str, business_details :: BusinessDetails, party_id :: Str, country_code :: Str }

fn credentials_role(role :: Str, details :: BusinessDetails, party_id :: Str, country_code :: Str) -> CredentialsRole {
  { role: role, business_details: details, party_id: party_id, country_code: country_code }
}

fn credentials_role_to_json(r :: CredentialsRole) -> jv.Json {
  JObj([("role", JStr(r.role)), ("business_details", business_details_to_json(r.business_details)), ("party_id", JStr(r.party_id)), ("country_code", JStr(r.country_code))])
}
...
type Credentials = { token :: Str, url :: Str, roles :: List[CredentialsRole] }
...
fn to_json(c :: Credentials) -> jv.Json {
  JObj([("token", JStr(c.token)), ("url", JStr(c.url)), ("roles", JList(list.map(c.roles, credentials_role_to_json)))])
}
...
fn business_details_schema() -> s.ModelSchema {
  { title: "BusinessDetails", description: "OCPI — BusinessDetails", fields: [s.required_str("name", [StrNonEmpty, StrMaxLen(100)]), s.optional(s.required_str("website", [StrMaxLen(255)]))] }
}

fn credentials_role_schema() -> s.ModelSchema {
  { title: "CredentialsRole", description: "OCPI 2.2.1 — Credentials role entry", fields: [s.required_str("role", [StrOneOf(role.all_roles_v221())]), s.required_object("business_details", business_details_schema()), s.required_str("party_id", [StrNonEmpty, StrMaxLen(3)]), s.required_str("country_code", [StrNonEmpty, StrMaxLen(2)])] }
}

fn credentials_schema_v221() -> s.ModelSchema {
  { title: "Credentials", description: "OCPI 2.2.1 — Credentials object", fields: [s.required_str("token", [StrNonEmpty, StrMaxLen(64)]), s.required_str("url", [StrNonEmpty, StrMaxLen(255)]), s.required_array("roles", KObject(credentials_role_schema()), [ListNonEmpty])] }
}
