...
type ClientError = HttpFailed(Str) | HttpStatus({ code :: Int, retry_after_ms :: Option[Int] }) | BadEnvelope(Str) | OcpiError(env.OcpiResponse)
...
fn with_token(req :: HttpRequest, token_b64 :: Str) -> HttpRequest {
  http.with_header(req, h.h_authorization(), str.concat("Token ", token_b64))
}
...
fn with_party_routing(req :: HttpRequest, from_party :: party.PartyId, to_party :: party.PartyId) -> HttpRequest {
...
fn send(req :: HttpRequest) -> [net] Result[jv.Json, ClientError] {
  match http.send(req) {
    ...
    Ok(resp) => if resp.status >= 200 and resp.status < 300 {
      ...
      Err(HttpStatus({ code: resp.status, retry_after_ms: parse_retry_after_ms(resp.headers) }))
    },
  }
}
...
fn put_json(url :: Str, body :: Str, token_b64 :: Str) -> [net] Result[jv.Json, ClientError] {
  let req := with_json_body(with_token(base_request("PUT", url), token_b64), body)
  send(req)
}

fn post_json(url :: Str, body :: Str, token_b64 :: Str) -> [net] Result[jv.Json, ClientError] {
  let req := with_json_body(with_token(base_request("POST", url), token_b64), body)
  send(req)
}

fn patch_json(url :: Str, body :: Str, token_b64 :: Str) -> [net] Result[jv.Json, ClientError] {
  let req := with_json_body(with_token(base_request("PATCH", url), token_b64), body)
  send(req)
}
...
# is silently clamped.
fn send_with_retry(req :: HttpRequest, policy :: RetryPolicy) -> [net, time] Result[jv.Json, ClientError] {
  retry_loop(req, policy, 1)
}

fn retry_loop(req :: HttpRequest, policy :: RetryPolicy, attempt :: Int) -> [net, time] Result[jv.Json, ClientError] {
  match send(req) {
    Ok(j) => Ok(j),
...
fn send_with_events(req :: HttpRequest, policy :: RetryPolicy, observer :: (RetryEvent) -> [io] Unit) -> [net, time, io] Result[jv.Json, ClientError] {
  retry_loop_events(req, policy, observer, 1)
}

fn retry_loop_events(req :: HttpRequest, policy :: RetryPolicy, observer :: (RetryEvent) -> [io] Unit, attempt :: Int) -> [net, time, io] Result[jv.Json, ClientError] {
  match send(req) {
    Ok(j) => Ok(j),
    Err(e) => if is_retryable(e) == false or attempt >= policy.max_attempts {
...
fn handshake(peer_versions_url :: Str, our_token :: Str, our_credentials :: jv.Json) -> [net] Result[jv.Json, ClientError] {
  match get_with_token(peer_versions_url, our_token) {
    Err(e) => Err(e),
    Ok(_versions) => {
      let creds_url := str.concat(derive_credentials_url(peer_versions_url), "/credentials")
