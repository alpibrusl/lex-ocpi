...
fn emit_top(j :: jv.Json) -> Result[Str, Str] {
  ...
  match emit_fields(j) {
    Err(msg) => Err(msg),
    Ok(fields) => Ok(render_module(name, fn_name, validator_name, title, desc, fields)),
  }
}
...
fn required_names(j :: jv.Json) -> List[Str] {
  match jv.get_field(j, "required") {
    ...
    Some(rs) => match jv.as_list(rs) {
      None => [],
      Some(items) => list.fold(items, [], fn (acc :: List[Str], v :: jv.Json) -> List[Str] {
        match jv.as_str(v) {
          None => acc,
...
fn is_required(name :: Str, required :: List[Str]) -> Bool {
  list.fold(required, false, fn (acc :: Bool, r :: Str) -> Bool {
    if acc {
      true
    } else {
      name == r
...
fn emit_each_field(entries :: List[(Str, jv.Json)], required :: List[Str]) -> Result[Str, Str] {
  list.fold(entries, Ok(""), fn (acc :: Result[Str, Str], pair :: (Str, jv.Json)) -> Result[Str, Str] {
    match acc {
      Err(e) => Err(e),
      Ok(so_far) => match emit_one_field(pair, required) {
        Err(e2) => Err(e2),
        Ok(line) => Ok(str.concat(so_far, str.concat("      ", str.concat(line, ",\n")))),
      },
    }
  })
}

fn emit_one_field(pair :: (Str, jv.Json), required :: List[Str]) -> Result[Str, Str] {
  match pair {
    (name, schema) => {
...
fn field_call(name :: Str, schema :: jv.Json) -> Result[Str, Str] {
  ...
  if ty == "string" {
    Ok(emit_str_field(name, schema))
  } else {
    if ty == "integer" {
      Ok(emit_int_field(name, schema))
    } else {
      if ty == "number" {
        Ok(emit_float_field(name, schema))
      } else {
        if ty == "boolean" {
          Ok(emit_bool_field(name))
        } else {
          if ty == "array" {
            emit_array_field(name, schema)
          } else {
            Err(str.concat("unsupported field type for ", str.concat(name, str.concat(": ", ty))))
          }
        }
      }
    }
...
fn emit_str_field(name :: Str, schema :: jv.Json) -> Str {
  ...
  str.concat("s.required_str(", str.concat(quote(name), str.concat(", [", str.concat(checks, "])"))))
}

fn emit_int_field(name :: Str, schema :: jv.Json) -> Str {
  let checks := int_checks(schema)
  str.concat("s.required_int(", str.concat(quote(name), str.concat(", [", str.concat(checks, "])"))))
}

fn emit_float_field(name :: Str, _schema :: jv.Json) -> Str {
  str.concat("s.required_float(", str.concat(quote(name), ", [])"))
}
  ...
  match jv.get_field(schema, "items") {
    ...
    Some(items) => match element_kind(items) {
      ...
      Ok(kind) => {
        ...
        Ok(str.concat("s.required_array(", str.concat(quote(name), str.concat(", ", str.concat(kind, str.concat(", [", str.concat(checks, "])")))))))
      },
    },
  }
}
...
fn element_kind(items :: jv.Json) -> Result[Str, Str] {
  ...
  if ty == "string" {
    Ok(str.concat("KStr([", str.concat(str_checks(items), "])")))
  } else {
    if ty == "integer" {
      Ok(str.concat("KInt([", str.concat(int_checks(items), "])")))
    } else {
      if ty == "number" {
        Ok("KFloat([])")
      } else {
        if ty == "boolean" {
          ...
          if ty == "object" {
            Ok("KObject(/* nested — define separately */)")
          } else {
            Err(str.concat("unsupported element type: ", ty))
          }
        }
      }
    }
...
fn str_checks(schema :: jv.Json) -> Str {
  ...
  let with_maxlen := match jv.get_field(schema, "maxLength") {
    ...
    Some(n) => match jv.as_int(n) {
      None => with_minlen,
      Some(v) => list.concat(with_minlen, [str.concat("StrMaxLen(", str.concat(int.to_str(v), ")"))]),
    },
  }
    ...
    Some(n) => match jv.as_int(n) {
      ...
      Some(v) => if v == 0 {
        ...
        if v == 1 {
          ...
          [str.concat("IntMin(", str.concat(int.to_str(v), ")"))]
        }
      },
    },
  }
...
fn enum_list(items :: jv.Json) -> Option[Str] {
  match jv.as_list(items) {
    None => None,
    Some(xs) => {
      let quoted := list.map(xs, fn (v :: jv.Json) -> Str {
        match jv.as_str(v) {
          None => "\"\"",
          Some(s) => quote(s),
        }
...
fn join_commas(parts :: List[Str]) -> Str {
  list.fold(parts, "", fn (acc :: Str, p :: Str) -> Str {
    if str.is_empty(acc) {
      p
    } else {
      str.concat(acc, str.concat(", ", p))
    }
  })
}
...
fn is_upper(c :: Str) -> Bool {
  c == "A" or c == "B" or c == "C" or c == "D" or c == "E" or c == "F" or c == "G" or c == "H" or c == "I" or c == "J" or c == "K" or c == "L" or c == "M" or c == "N" or c == "O" or c == "P" or c == "Q" or c == "R" or c == "S" or c == "T" or c == "U" or c == "V" or c == "W" or c == "X" or c == "Y" or c == "Z"
}

# ---- Output rendering -------------------------------------------
fn render_module(_name :: Str, schema_fn :: Str, validator_fn :: Str, title :: Str, description :: Str, fields_block :: Str) -> Str {
  let header := "# Generated by lex-ocpi/tools/gen.lex — review before committing.\n\n"
  let schema_open := str.concat("fn ", str.concat(schema_fn, "() -> s.ModelSchema {\n  {\n    title: "))
  let titles := str.concat(quote(title), str.concat(",\n    description: ", str.concat(quote(description), ",\n    fields: [\n")))
  let schema_close := "    ],\n  }\n}\n\n"
  let validator := str.concat("fn ", str.concat(validator_fn, str.concat("(j :: jv.Json) -> Result[jv.Json, List[e.Error]] {\n  s.validate(", str.concat(schema_fn, "(), j)\n}\n"))))
  str.concat(header, str.concat(schema_open, str.concat(titles, str.concat(fields_block, str.concat(schema_close, validator)))))
}
